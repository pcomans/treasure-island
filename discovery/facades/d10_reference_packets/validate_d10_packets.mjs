...
const IDS = Object.freeze([
  "w96665924", "w96665925", "w96665928", "w96665933", "w96665935",
  "w96665936", "w96698611", "w96698619", "w96698627", "w96698634",
  "w96698641", "w96698643", "w96698645", "w96698648", "w96698650",
]);
const PACKET_FILENAMES = Object.freeze({
  w96665924: "w96665924_849_avenue_d_candidate.md",
  w96665925: "w96665925_1107_bigelow_court.md",
  w96665928: "w96665928_1117_keppler_court.md",
  w96665933: "w96665933_1105_bigelow_court.md",
  w96665935: "w96665935_1135_mason_court.md",
  w96665936: "w96665936_1128_reeves_court.md",
  w96698611: "w96698611_unaddressed_courtyard_building.md",
  w96698619: "w96698619_1254_625_13th_street.md",
  w96698627: "w96698627_1216_1430_gateview_court.md",
  w96698634: "w96698634_1253_exposition_drive.md",
  w96698641: "w96698641_unaddressed_avenue_d_building.md",
  w96698643: "w96698643_1249_exposition_drive.md",
  w96698645: "w96698645_1214_1420_gateview_court.md",
  w96698648: "w96698648_1252_exposition_drive.md",
  w96698650: "w96698650_unaddressed_avenue_h_building.md",
});
const COMPONENT_READY_IDS = new Set(["w96665925", "w96665928", "w96698611", "w96698634"]);
const APPEARANCE_BLOCKED_IDS = new Set(["w96665933", "w96698650"]);
...
const PACKET_TIME_STATE = Object.freeze({
  commit: "a37c0c6e16ab7fef3e767c2367024abd3e563091",
  recognition_metric: "5/213",
});
const AUTHORITY = Object.freeze({
  ...
  logical: { path: "generated/world/logical-objects.json", schema: "ti.godot-world/2", sha256: "5a74a80a7930ef8edfbea4f7c3a15a509d38e59b64e55d17a26ea1061137ba11" },
...
const PRIOR_PACKET_DIRS = Object.freeze([
  ...
  "d7_reference_packets", "d8_reference_packets", "d9_reference_packets",
]);
...
function contracts() {
  ...
  invariant(priorIds.size === 151, `Expected 151 unique prior packet IDs, found ${priorIds.size}`);
  ...
  invariant(JSON.stringify(derivedCatalog) === JSON.stringify(IDS), `D10 current catalog cohort/order drifted: ${derivedCatalog.join(", ")}`);
  invariant(JSON.stringify(derivedRegistry) === JSON.stringify(IDS), `D10 current registry cohort/order drifted: ${derivedRegistry.join(", ")}`);
...
invariant(
  ...
  "D10 packet-time v4 receipts and current v6 binding authorities must remain explicitly separate",
);
...
const packetTimeCatalogRow = `| D10 packet-time catalog snapshot receipt | \`${PACKET_TIME_CATALOG_SNAPSHOT.schema}\`; \`${PACKET_TIME_CATALOG_SNAPSHOT.sha256}\` | immutable packet-time provenance; same bytes as the then-current catalog at seal time |`;
const packetTimeRow = `| D10 packet-time registry snapshot receipt | \`${PACKET_TIME_REGISTRY_SNAPSHOT.schema}\`; \`${PACKET_TIME_REGISTRY_SNAPSHOT.sha256}\` | immutable packet-time provenance; same bytes as the then-current registry at seal time |`;
const packetTimeMetricRow = `| D10 packet-time recognition rollup | \`${PACKET_TIME_STATE.recognition_metric}\` | immutable five-unit acceptance state at seal time; not a claim about current compiler output |`;
const packetTimeCommitRow = `| D10 packet-time commit | \`${PACKET_TIME_STATE.commit}\` | immutable repository state at packet seal time |`;
...
invariant(readme.includes(packetTimeCatalogRow), "README lost exact D10 packet-time catalog receipt");
invariant(readme.includes(packetTimeRow), "README lost exact D10 packet-time registry receipt");
invariant(readme.includes(packetTimeMetricRow), "README lost exact D10 packet-time recognition rollup");
invariant(readme.includes(packetTimeCommitRow), "README lost exact D10 packet-time commit");
invariant(readme.includes(currentCatalogRow), "README lost separately labeled current catalog binding authority");
invariant(readme.includes(currentRow), "README lost separately labeled current registry authority");

const expectedFiles = new Set(["README.md", "validate_d10_packets.mjs", ...Object.values(PACKET_FILENAMES)]);
const actualFiles = readdirSync(HERE);
invariant(actualFiles.length === expectedFiles.size, `Expected ${expectedFiles.size} D10 files, found ${actualFiles.length}`);
for (const filename of actualFiles) invariant(expectedFiles.has(filename), `Unexpected D10 file (including binary/image): ${filename}`);
...
for (const contract of localContracts) {
  ...
  const fieldTokens = [
    sourceKey, `v${object.source.osm_version}`, object.source.osm_timestamp, AUTHORITY.osm.sha256,
    "No proper name exists in the frozen source.",
    ...
    "standalone direct footprint; no part child",
    PACKET_TIME_REGISTRY_SNAPSHOT.sha256,
  ];
  ...
  if (!tags["addr:housenumber"] && !tags["addr:street"]) invariant(packet.includes("No frozen street address exists"), `${filename} lacks frozen-address absence`);
  ...
  invariant(packet.includes("Packet-time catalog/registry provenance:"), `${filename} does not label its catalog/registry hashes as packet-time`);
  invariant(!packet.includes("separately checked current registry"), `${filename} still mislabels packet-time registry bytes as current`);
...
invariant(readySeen === 9 && componentSeen === 4 && blockedSeen === 2, `Readiness counts drifted: ${readySeen}/${componentSeen}/${blockedSeen}`);
invariant(fieldChecks === 300 && facingChecks === 60 && links === 15, `Audit totals drifted: ${fieldChecks}/${facingChecks}/${links}`);
invariant(readme.includes("Totals: **9 prototype-ready / 4 component-ready / 2 appearance-blocked**"), "README readiness totals drifted");
for (const authority of Object.values(AUTHORITY)) {
...
for (const sourceId of ["CITY-EAS", "CITY-XWALK", "GOOGLE-SV"]) invariant(readme.includes(`\`${sourceId}\``), `README lacks source ${sourceId}`);

process.stdout.write(`D10 packet audit: PASS (15 IDs/order/direct bindings, 45 recomputed hashes, ${fieldChecks} exact field checks, ${facingChecks} facing partitions, ${links} packet links, readiness ${readySeen}/${componentSeen}/${blockedSeen}; immutable packet-time catalog/registry ${PACKET_TIME_CATALOG_SNAPSHOT.sha256}/${PACKET_TIME_REGISTRY_SNAPSHOT.sha256}, ${PACKET_TIME_STATE.recognition_metric}, commit ${PACKET_TIME_STATE.commit}; current v6 catalog/registry ${fileSha256(AUTHORITY.catalog.path)}/${fileSha256(AUTHORITY.registry.path)})\n`);
