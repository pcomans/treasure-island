...
invariant(
  ...
  "D10 packet-time v4 receipts and current v7 binding authorities must remain explicitly separate",
);
...
process.stdout.write(`D10 packet audit: PASS (15 IDs/order/direct bindings, 45 recomputed hashes, ${fieldChecks} exact field checks, ${facingChecks} facing partitions, ${links} packet links, readiness ${readySeen}/${componentSeen}/${blockedSeen}; immutable packet-time catalog/registry ${PACKET_TIME_CATALOG_SNAPSHOT.sha256}/${PACKET_TIME_REGISTRY_SNAPSHOT.sha256}, ${PACKET_TIME_STATE.recognition_metric}, commit ${PACKET_TIME_STATE.commit}; current v7 catalog/registry ${fileSha256(AUTHORITY.catalog.path)}/${fileSha256(AUTHORITY.registry.path)})\n`);
