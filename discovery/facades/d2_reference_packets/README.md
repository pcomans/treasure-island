# D2 address-only reference-readiness packets

Checked: **2026-09-04**

Scope: **15 frozen address-only residential units; research and bounded art
handoff only**

> **2026-09-07 authority addendum:** the packet body below remains the frozen
> research boundary. A later independently reviewed production transaction
> accepts exactly 1441 Chinook Court (`w95934105`) as the ninth recognizable
> physical unit. Current source authority is compiler `1.8.0`, catalog/runtime
> registry v9, adapter contracts/loader API v8, and `9/213`; the other fourteen
> D2 research units remain unaccepted. The accepted 1441 record binds the
> immutable `-009` production packet and both independent reviews through a
> distinct catalog transaction, without rewriting this research history or
> importing panorama pixels.

This directory records a receiver-first pass for exactly ten Chinook Court,
three Striped Bass Street, and two Croaker Court buildings. It does not change
the recognition catalog, runtime registry, generated world, prototypes, or live
attachments. Each packet separates `verified_fact`, `reference_observation`,
reversible `production_inference`, and `unknown`.

## Readiness ledger

| Unit | Frozen address | Exact identity / lifecycle | Target exterior evidence | Readiness |
| --- | --- | --- | --- | --- |
| `w95934105` | 1441 Chinook Court | City EAS `725381`; Navy/Swords; paired 2025 project name | Jul 2023 SSE public side | **target-side prototype-ready** |
| `w95934107` | 1420 Striped Bass Street | City EAS `725272`; Navy/Villages; 2025 managed list | none | **appearance-blocked** |
| `w95934112` | 1418 Striped Bass Street | City EAS `725270`; Navy/Villages; 2025 managed list | none | **appearance-blocked** |
| `w95934113` | 1437 Chinook Court | City EAS `725379`; removed from managed premises 2026-02-01 | none | **appearance/lifecycle-blocked** |
| `w95934117` | 1444 Croaker Court | City EAS `725327`; Navy/Villages; 2025 managed list | Jul 2023 WSW public side | **target-side prototype-ready** |
| `w95934118` | 1419 Striped Bass Street | City EAS `725271`; Navy/Catholic; 2022 balcony report | no target-identifiable view | **appearance-blocked** |
| `w95934120` | 1449 Croaker Court | City EAS `725328`; removed from managed premises 2026-02-01 | none | **appearance/lifecycle-blocked** |
| `w95934121` | 1445 Chinook Court | City EAS `725383`; HR360 sublease through 2027, terminable early | none | **appearance-blocked** |
| `w95934129` | 1440 Chinook Court | City EAS `725386`; HR360 sublease through 2027, terminable early | none | **appearance-blocked** |
| `w95934131` | 1438 Chinook Court | City EAS `725385`; Navy/Villages; 2025 managed list | none | **appearance-blocked** |
| `w95934137` | 1436 Chinook Court | City EAS `725378`; Navy/Villages; 2025 managed list | none | **appearance-blocked** |
| `w95934143` | 1443 Chinook Court | City EAS `725382`; paired 2025 project name; exact permit components | rear component records, no mapped view | **target-component study-ready** |
| `w95934144` | 1439 Chinook Court | City EAS `725380`; Navy/Villages; 2025 managed list | Jul 2023 SSE public side | **target-side prototype-ready** |
| `w96665886` | 1447 Chinook Court | City EAS `725384`; HR360 sublease through 2027, terminable early | none | **appearance-blocked** |
| `w96665900` | 1442 Chinook Court | City EAS `725387`; HR360 sublease through 2027, terminable early | none | **appearance-blocked** |

Counts: **15/15** exact frozen-source/direct-receiver packets; **15/15** exact
live City address records; **3/15** target-identifiable dated public-side
observations; **1/15** additional target-specific component study; **11/15**
have no safe target appearance prototype. None is accepted or attached by this
research batch.

## Authoritative corrections and lifecycle boundary

- The live San Francisco Enterprise Addressing System (EAS) has one exact
  record for every D2 address, all on parcel `1939001`. An active address point
  proves civic identity, not that its building survives or is occupied.
- The Navy's 2015 Site 12 table binds all 15 addresses and then-current housing
  providers. Its Figure 5 binds unit positions and garages. It contains no
  verified exterior elevation usable as facade evidence.
- Navy's 2020 review supports only the cohort grammar: planned early-1960s / 1965
  construction, two-storey slab-on-grade 4/6/8-unit housing, backyards, grass,
  paved roads and parking. It cannot be transferred as an exact opening schedule.
- TIDA's 2026 John Stewart Company amendment proves `1437 Chinook Court` and
  `1449 Croaker Court` were vacant buildings removed from those managed premises
  effective 2026-02-01. It does **not** prove demolition.
- `1436`, `1438`, `1439`, `1444`, `1418`, and `1420` appear on the agreement's
  2025 managed-building list/map. `1440`, `1442`, `1445`, and `1447` appear in
  TIDA's 2024 HR360 sublease through 2027, subject to early termination. These
  are lifecycle facts, not exterior observations.
- Provider/program groupings changed over time. They must not become invented
  signs or color schemes.

## Shared primary-source index

| ID | Primary source | Supports | Does not support |
| --- | --- | --- | --- |
| `CITY-EAS` | [SF Enterprise Addressing System](https://data.sfgov.org/d/3mea-di5p) and [API](https://data.sfgov.org/resource/3mea-di5p.json) | live exact address IDs, coordinates, parcel | standing structure, occupancy, facade |
| `NAVY-S12-2015` | [Final Survey Completion Report, Site 12 housing](https://media.defense.gov/2022/Mar/22/2002960622/-1/-1/0/TI_2015_FINAL_SURVEY_COMPLETION_REPORT_RAD_SCOPING%20SURVEYS_OF_IR_SITE12_HOUSING_UNITS_PART_1OF_9.PDF) | exact address/provider table and Figure 5 position plan | current exterior or transferable openings |
| `NAVY-5YR-2020` | [Second Five-Year Review](https://media.defense.gov/2022/Mar/25/2002963837/-1/-1/0/TI_20200805_SECOND_FIVE_YEAR_REVIEW%20%282%29.PDF) | Site 12 housing history, two-storey/slab/4-8-unit cohort, status mapping | target facade detail |
| `TIDA-JSCO-2026` | [Eighth Amendment / property-management agreement](https://media.api.sf.gov/documents/011426_6j_JSCO.pdf) | 2025 managed list/map; 1437/1449 removal on 2026-02-01 | reason, demolition, facade |
| `TIDA-HR360-2024` | [HR360 sublease](https://media.api.sf.gov/documents/061224_Item_6c_Healthright_360.pdf) | 1440/1442/1445/1447 lease cohort and bounded term | occupancy of a particular unit or facade |
| `MOHCD-AMR-2025` | [MOHCD-financed-project deadlines](https://media.api.sf.gov/documents/2025_AMR_Deadlines_for_MOHCD-Financed_Projects.pdf) | 1441+1443 project label and current administrative grouping | a visible building sign or facade |
| `SF-PERMITS` | [SF Building Permits dataset](https://data.sfgov.org/d/i98e-djp9) | exact permit descriptions, including 1443 rear patio/balcony work | a surveyed elevation or unchanged current state |
| `TIDA-UTIL-2026` | [March 2026 Director's Report](https://media.api.sf.gov/documents/031126_Directors_Report.pdf) | residential service remained on Chinook/Croaker streets | exact unit survival/appearance |

The 2014 City rentable-unit packet is useful corroboration for the Villages
subset: [Board packet](https://sfgov.legistar.com/View.ashx?GUID=24073A24-1CB2-4921-AE64-3318CB899E6B&ID=3246246&M=F).

Link check: the City EAS/API, permit dataset, TIDA JSCo/HR360/Director's Report,
and Legistar endpoints returned HTTP 200 after redirects on 2026-09-04. The two
Navy PDFs and MOHCD PDF returned HTTP 403 to a generic HEAD client but were
indexed/read through the web research path; that client distinction is retained
instead of misreporting the sources as broken or independently downloadable.

## Dated public observations

Google-hosted contributor panoramas by `TyFlow io`, all dated **July 2023**:

- `D2-PANO-A`, pano `CIHM0ogKEICAgICpvKnr6gE`, viewpoint
  `37.8237151,-122.3738188`: [request view](https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=37.8237151,-122.3738188&heading=342&pitch=0&fov=75).
- `D2-PANO-B`, pano `CIHM0ogKEICAgICpvM6o_wE`, viewpoint
  `37.8241564,-122.3753174`: [request view](https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=37.8241564,-122.3753174&heading=78&pitch=0&fov=75).
- `D2-PANO-C`, pano `CIHM0ogKEICAgICpvKmj_wE`, viewpoint
  `37.8235690,-122.3741335`: [request view](https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=37.8235690,-122.3741335&heading=322&pitch=0&fov=75).

The repository retains only URLs, IDs, dates and written observations. No
panorama pixels may be copied, embedded, trained into, or shipped as textures.

## Cohort grammar and recognition gate

The exact footprint family itself is useful: most units are similarly sized,
zigzagged row blocks with alternating long faces, recessed links and projecting
ends. The observed subset adds shallow dark-shingle gables, deep eaves, muted
stucco fields, paired rectangular windows, and recessed two-level balcony or
breezeway voids. That is a **shared cohort grammar**, never evidence for an
unseen target.

A D2 building can pass only through at least two coherent, target-bound cues at
a mapped side, while remaining distinguishable from the other 14 D2 buildings
and the adjacent Halibut/Sturgeon/Gateview rows. An address plaque, provider
name, color swatch, generic balcony, random damage, or exact footprint alone
does not pass. Tileable generated stucco/shingle fields are encouraged; an
imperfect seam is acceptable in a prototype, but low-frequency weathering and
opening/balcony hierarchy must prevent obvious wallpaper repetition.

## Packet index

- [1441 Chinook](w95934105_1441_chinook_court.md)
- [1420 Striped Bass](w95934107_1420_striped_bass_street.md)
- [1418 Striped Bass](w95934112_1418_striped_bass_street.md)
- [1437 Chinook](w95934113_1437_chinook_court.md)
- [1444 Croaker](w95934117_1444_croaker_court.md)
- [1419 Striped Bass](w95934118_1419_striped_bass_street.md)
- [1449 Croaker](w95934120_1449_croaker_court.md)
- [1445 Chinook](w95934121_1445_chinook_court.md)
- [1440 Chinook](w95934129_1440_chinook_court.md)
- [1438 Chinook](w95934131_1438_chinook_court.md)
- [1436 Chinook](w95934137_1436_chinook_court.md)
- [1443 Chinook](w95934143_1443_chinook_court.md)
- [1439 Chinook](w95934144_1439_chinook_court.md)
- [1447 Chinook](w96665886_1447_chinook_court.md)
- [1442 Chinook](w96665900_1442_chinook_court.md)
