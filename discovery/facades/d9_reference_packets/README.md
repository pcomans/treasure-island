...
> “schema-v7” below mean the last validated D9 packet boundary, not current
...
> D9 validator remains an intentionally unmigrated snapshot.
...
| `discovery/facades/facade-recognition-catalog.json` (current checkout) | `ti.facade-recognition-catalog/7`; SHA-256 emitted by validator | used only to rederive the exact 15-ID cohort and order |
| `game/resources/facades/facade-runtime-registry.json` (current checkout) | `ti.facade-runtime-registry/7`; SHA-256 emitted by validator | used only to verify those 15 current direct bindings; global counts and recognition rollup remain compiler-owned |
...
immutable receipts while separately loading and hashing the current schema-v7
...
immutable; the current schema-v7 catalog/registry reproduce the exact 15-ID
