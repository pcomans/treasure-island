...
W=Path('/Volumes/Macintosh_HD/Users/user302070/Documents/Codex/2026-09-08/start-from-commit-f377dcac-and-read/work/northern-canopy-free-family-pilot-001/1237-1229-mechanics-001/1229');N=Path('/private/tmp/1237-1229-mechanics-001/1229');P=N/'1229-mechanics.avi';sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
r=json.loads((W.parent/'native-mechanics-1229-001-execution.json').read_text());assert r['status']=='terminal' and r['pid_absent'] and r['pins_unchanged'],'Actual writer terminal/input preservation required; rawFAIL originals remain evidence'
release=json.loads((W.parent/'SLOT_RELEASE-native-mechanics-1229-001.json').read_text());assert release['slot_released'] and not release['engine_processes'],'Engine release required before offline finalization'
...
result_path=W.parent/'native-mechanics-1229-001-result.json';m=json.loads(result_path.read_text())if result_path.is_file()else dict(failures=['No native result produced'],captures=[],motion={})
