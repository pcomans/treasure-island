...
try:
 ...
 if (M/(name+'-execution.json')).exists():
  ...
  movie=Path('/private/tmp/1237-1229-mechanics-001/1237/1237-mechanics.avi')
  if movie.is_file():
   finalize=subprocess.run(['/usr/bin/python3','-B',str(C/'finalize-1237.py')],cwd=str(C)).returncode
