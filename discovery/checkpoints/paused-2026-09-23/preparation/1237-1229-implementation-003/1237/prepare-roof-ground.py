...
W=Path(__file__).resolve().parent;R=Path('/Volumes/Macintosh_HD/Users/user302070/Documents/Codex/2026-09-08/start-from-commit-f377dcac-and-read/work/northern-canopy-free-family-pilot-001/1237-1229-implementation-003/staged');config=R/'game/resources/facades/northpoint_1237_study.json'
...
layout=dict(source_chunk_sha256=sha(R/'generated/world/chunks/x_-1__z_-4.json'),source_osm_sha256=sha(R/'data/osm/treasure-island-2026-08-27.osm'),global_frame=dict(origin_xz=origin,tangent_xz=t,normal_xz=n,station_limits_m=[start,end],footway_depth_m=[walk_near,walk_far]),source_road_profile=profile,entries=entries,source_collision_unchanged=True,ground_source_chunks=["x_-1__z_-4","x_-2__z_-4"],parking_outer_rim_changes=rim_changes,parking_overlap_m=overlap_m,road_corner_max_jump_m=max_jump,road_edge_max_depth_per_station=max_edge_slope,native_coordinate_ulp_m=coordinate_ulp_m,ground_top_triangles=dict(footway=len(walk['top_triangles']),parking=len(parking['top_triangles']),entries=sum(len(e['path_mesh']['top_triangles'])for f in cfg['target']['frames']for e in f['entries'])),roof_bands=roof_bands,scope='First coherent1221-only source/ground/roof geometry; no engine/native/visual acceptance')
