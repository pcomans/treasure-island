...
rows=json.loads((R/'generated/world/chunks/x_-1__z_-4.json').read_text())['records']
records={x['object_key']:x for x in rows}
adj=json.loads((R/'generated/world/chunks/x_-2__z_-4.json').read_text())['records']
def merge(prefix):
...
cfg=json.loads((ORIGINAL/'northpoint_1237_study.json').read_text())
...
(R/'game/resources/facades/northpoint_1237_study.json').write_text(json.dumps(cfg,indent=2)+'\n')
