...
config_path=R/'game/resources/facades/northpoint_1229_study.json';cfg=json.loads(config_path.read_text());chunk=R/'generated/world/chunks/x_-2__z_-4.json';records=json.loads(chunk.read_text())['records'];wall=next(x for x in records if x['object_key']=='building:w96665911:wall')
...
# TARGET_CHECKS
roof=next(x for x in records if x['object_key']=='building:w96665911:roof')
check(cfg['mapped_runs']==[2, 4, 5, 6, 8] and cfg['protected_runs']==[0, 1, 3, 7, 9, 10, 11],'Exact independent candidate and protected partition')
check(len(wall['vertices'])==144 and len(wall['indices'])==72 and len(roof['vertices'])==24 and len(roof['indices'])==18,'Exact target wall roof cardinality')
...
for v in json.loads((W/'view-plan.json').read_text())['views']:
 ...
 okay=under is not None and camera[1]-under[0]>.2 and segment is None and hit is not None and hit[1] in ['building:w96665911:wall','building:w96665911:roof'];check(okay,'Source-only stock-camera placement '+v['id']);camera_rows.append(dict(id=v['id'],camera=camera,target_hit=hit,segment_hit=segment,source_camera_clear=okay))
