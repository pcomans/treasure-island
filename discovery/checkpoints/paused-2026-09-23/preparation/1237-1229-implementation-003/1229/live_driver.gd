...
const OUTPUT := "/private/tmp/1237-1229-study-001/1229"
	...
	for frame: Dictionary in config.target.frames:
		...
		for spec: Dictionary in specs:
			...
			if bool(spec.eligible):
				passed = passed and metadata.get("source_keys", []) == ["w96665911"] and absf((hit.get("normal", Vector3.ZERO) as Vector3).y) <= .3 and (hit.get("position", Vector3.ZERO) as Vector3).distance_to(front) < .003 and (study_root.get_node("ProjectedUpperSiding") as MeshInstance3D).layers == 2
