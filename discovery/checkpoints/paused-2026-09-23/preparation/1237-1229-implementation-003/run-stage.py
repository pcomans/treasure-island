...
name=sys.argv[1];plan=json.loads((p/'run-plan.json').read_text())[name]
...
if name=='import-001':
 ...
 r['post_import_outputs']={str(q):digest(q) if q.is_file() else None for q in [cache,repo/'game/scripts/world/facades/northpoint_1237_study.gd.uid',repo/'game/scripts/world/facades/northpoint_1229_study.gd.uid']}
