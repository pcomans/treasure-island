...
## Owner resumption and quota wind-down — 2026-09-23
...
Check weekly quota hourly. Once verified remaining weekly quota is **below 40%**, stop new dispatch; finish only bounded in-flight work when safe, otherwise preserve a safe checkpoint and actual live handles. Do not restart work or lose completed stages. Preserve independent source/mechanical, visual and release gates, commit and push the appropriate authorized checkpoint to **MAIN**, then pause autonomous work and the existing quota monitor. Do not automatically resume when quota resets; wait for a new owner instruction. Missing or stale quota evidence is a limitation to report, never a fabricated percentage. Root remains orchestrator and delegates execution throughout.
