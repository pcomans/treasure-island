...
At meaningful work checkpoints, ordinarily after a few substantial work rounds and earlier when repeated churn appears, `/root` assigns a named independent subagent to evaluate other subagents' efficiency. Sample relevant task/tool activity and `discovery/RETRO_LOG.md`; distinguish necessary evidence and acceptance checks from repeated low-value cycles, duplicated work, or avoidable waits. Keep the review bounded and its findings concise and evidence-backed. The owner subsequently replaced the progress reminder with an hourly weekly-quota guard. Keep unchanged-state checks quiet; do not turn wakeups into independent audits or add timers or a reporting framework.

A same-task Codex heartbeat defers while its target turn is unfinished. During active owner-authorized work, `/root` must check verified weekly quota at substantial round boundaries and at least hourly itself; the existing hourly heartbeat is the idle fallback, without a promised fire. Identify the weekly window by `window_minutes=10080`, retain the observation timestamp, and do not infer remaining quota from missing or stale data. Follow the wind-down rule below once 25% or less remains. Keep ordinary unchanged checks quiet and notify only threshold crossing, failure, completion or a needed decision. These checks add no independent audit, reporting framework or acceptance gate.
...
Use a small number of named agents with concrete targets, output locations, stop conditions and ownership. The existing role prompts and project skill are indexed in [the harness entrypoints](CLAUDE.md#persisted-entrypoints). Other harnesses can read these same files without inventing a callable agent configuration or copying local session state. An agent must not independently accept its own implementation or exported package; package or technical PASS does not supply the separate visual decision. Routine handoffs contain short status, actual changed paths, source revision, rendered evidence link/result and any blocker. Do not duplicate manifests or add JSON/hash/count reporting layers. Generate required metadata once and reference it; do not manually repeat totals or transcribe hashes in successive human updates. Include metadata only as needed by existing contracts. At formal readiness/review boundaries, retain the immutable full input-map path and SHA256 required by the [existing binding rule](LEARNINGS.md#construct-completed-evidence-records-once); this is not a requirement to create a new map for each ordinary artist update. Contract-level ceremony requires deliberate redesign, not ad hoc removal of required bindings or gates.
...
## Owner resumption and quota wind-down — updated 2026-09-24

The owner explicitly resumed improving the buildings identified by the 34-building audit, superseding the earlier inference-cost pause and narrower harness-only publication limit. Preserve the isolated 1232 study's unpromoted status until its required independent gates pass; resumption alone grants no production promotion or acceptance credit.

On 2026-09-24 the owner explicitly instructed, “Then keep working on the buildings until quota hits 25%.” This resumes bounded building work and supersedes the earlier 40% cutoff. Check verified weekly quota at substantial round boundaries and at least hourly. Once verified remaining weekly quota is **at or below 25%**, stop new building dispatch; finish only bounded in-flight work when safe, otherwise preserve a safe checkpoint and actual live handles. Do not restart work or lose completed stages. Preserve independent source/mechanical, visual and release gates, commit and push the appropriate authorized checkpoint to **experiment/housing-family-instances** (the current authorized branch; do not merge or publish MAIN), then pause autonomous building work and the existing quota monitor. Do not automatically resume when quota resets; wait for a new owner instruction. Missing or stale quota evidence is a limitation to report, never a fabricated percentage. Root remains orchestrator and delegates execution throughout.
