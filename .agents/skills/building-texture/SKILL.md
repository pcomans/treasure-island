...
7. A named Godot integration/test subagent reads [godot-material-contract.md](references/godot-material-contract.md) before importing or reviewing in Godot. It validates exact game-receiver ownership and the declared measured or inferred scale, including a close view and the ordinary gameplay camera, and reports technical/material findings. Send the actual receiver views to the separate independent visual reviewer for the absolute art decision; “better than a gray placeholder” is diagnostic only.
...
Use the project's [authoring and batch-delivery rules](../../../AGENTS.md#authoring-and-batch-delivery): ordinary authorized material/code edits should reach a source run/reload and actual gameplay render early. Reuse the complete proven driver and send the first coherent result to the separate visual reviewer; do not wait for export or release paperwork. Older accepted assets are a quality floor, and added detail alone is not improvement. Source/mechanical, visual and per-unit packet gates remain distinct even when several ready units share a candidate app. Export and package acceptance belong at the batch checkpoint, followed by serialized authority and a final exact-current release. A package PASS never supplies visual acceptance or self-grants credit.

For the specifically approved 1239 first-study ordering trial, use the [bounded project rule](../../../LEARNINGS.md#trial-1239-early-capture-ordering). Provisional source pixels may precede completed independent source/geometry review only within that authorized order; every applicable semantic, source, mechanics, separate visual and package gate remains required before acceptance.
