...
func _run() -> void:
	...
	if not _failed:
		print("PASS: Building 1's reviewed runs21..51 ivory field remains byte/topology bounded as historical evidence while the current 6-mesh hero wall supersedes it without duplicate render or collision ownership; world 735/940/954/64,118/466")
	_finish()
