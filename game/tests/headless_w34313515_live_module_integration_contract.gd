...
func _run() -> void:
	...
	if not _failed:
		print("PASS: exactly four independently KEEP_WITH_DOCUMENTED_LIMITATION w34313515 BAY module-atlas exemplars remain unchanged at WSW runs 0/6 and NNW runs 44/47; the distinct independently accepted w291196370 and w34313520 scopes remain isolated, and whole-island topology is 735/944/957/64,572/466/466 (playable rows/meshes/surfaces/triangles/bodies/shapes)")
	_finish()
...
func _protected_receivers_exclude_bay(builder: WorldChunkBuilder) -> bool:
	for receiver_key: String in PROTECTED_RECEIVERS:
		if receiver_key == CHAPEL_ISOLATION.WALL_KEY:
			if not CHAPEL_ISOLATION.exact_pair_excludes_module_family("W34313515LiveModules"):
				return false
			continue
		var record := _record_for(receiver_key)
		if record.is_empty():
