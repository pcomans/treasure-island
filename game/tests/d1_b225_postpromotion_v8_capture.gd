...
## Fresh postpromotion evidence for B225 under current v8 / 8-of-213 authority.
## This harness instantiates ordinary main.tscn and only observes the attachment
## already built by the canonical world path. It never loads or calls the
## historical candidate seam or the live adapter, never writes player/camera
## transforms after real input begins, and claims no additional recognition.
...
const B225_OUTPUT := "res://evidence/first-playable/d1-b225-postpromotion-v8-2026-09-05"
const REHEARSAL_OUTPUT := "user://d1-b225-postpromotion-v8-complete-rehearsal-2026-09-05"
const B225_GENERATOR := "res://game/tests/d1_b225_postpromotion_v8_capture.gd"
...
const FOCUSED_CONTRACT_PATH := "res://game/tests/headless_d1_b225_postpromotion_v8_contract.gd"
...
const CATALOG_SCHEMA_PATH := "res://discovery/facades/facade-recognition-catalog.schema.json"
...
const PACKAGE_RECEIPT_FILENAME := "package-verification-receipt.json"
const PACKAGE_RECEIPT_SCHEMA := "ti.d1-b225-postpromotion-v8-package-verification/1"
const FROZEN_PRODUCTION_MANIFEST_PATH := "res://evidence/first-playable/d1-b225-production-attachment-v7-staging-2026-09-05/capture-manifest.json"
const FROZEN_PRODUCTION_REVIEW_PATH := "res://evidence/reviews/d1-b225-production-attachment-v7-staging-2026-09-05-INDEPENDENT_VISUAL_REVIEW.md"
const FROZEN_PRODUCTION_TREE_SHA256 := "f42dbec489c6fda55b612aba20c99ee2233857cb60a1ce9c512d024b35d0dcb7"
...
const DEPENDENCY_HASHES := {
	...
	FOCUSED_CONTRACT_PATH: "fd63ced9552dd9a1749de0047d2cebd1b46a1b093ebe1645dd4fcace19186353",
	...
	CATALOG_SCHEMA_PATH: "44584e92e1652fc930f47882b4a83304f5216c14173eb805bc9ce68ea9927cac",
	ADAPTER_CONTRACTS_PATH: "dd2d13e3b0f6eee1f8c5f2957927c4f3caba43b31883beea925f9a91b826d65c",
	RUNTIME_LOADER_PATH: "12e6c64b23b0783ed240b3d6c02499c3940c2b6a14344caa29778b850db06c04",
	PACKAGE_CONTRACT_PATH: "6d2d05e28657b5de89bce0cea0c70819b791d886921f4a057c80cf548713cbcc",
	EXPORT_PRESET_PATH: "b9e991df5f586fc5ac038ff10815d5cf9569df5dea24b38dabeab959f0bcaee4",
	"res://game/tests/headless_startup_configuration_contract.gd": "891d59f5e062fb6d8fcb71d3f84f1b472d685cabc0cc21ad70e027294766bcc8",
	FROZEN_PRODUCTION_MANIFEST_PATH: "96c76fd99960f1345a7c56f7fc6678ac284f98cc601a3ba65ed57020491f18dc",
	FROZEN_PRODUCTION_REVIEW_PATH: "87dc2b9febf7110ccd5c1eabed1a290fea5900508561298dd7cf3b6b6fcb1d95",
}
...
func _run_b225() -> void:
	...
	if mode not in ["source-contract", "preflight", "review-stills", "review-telemetry", "review-motion", "stills", "telemetry", "visual-motion"]:
		_fail("Pass exactly --capture-mode=source-contract, preflight, review-stills, review-telemetry, review-motion, stills, telemetry, or visual-motion.")
		_finish(null)
		return
	if mode == "source-contract":
		if _require(_generator_matches(), "B225 candidate generator SHA argument is absent or stale.") \
		and _require(_dependencies_match(), "B225 postpromotion dependency closure drifted.") \
		and _require(_capture_time_authority_matches(_capture_time_authority_receipt()), "B225 current v8 authority snapshot failed.") \
		and _require(not FileAccess.file_exists(CAPTURE_OVERRIDE), "Temporary capture override must be absent during the source contract.") \
		and _require(_output_root_absent(B225_OUTPUT), "Postpromotion evidence output already exists before capture."):
			print("PASS: B225 postpromotion capture source contract is exact at current v8 / 8-of-213 with frozen v7 acceptance provenance, no downstream feedback, no override, and no existing packet")
		_finish(null)
		return
	...
	or not _require(_generator_matches(), "B225 candidate generator SHA argument is absent or stale.") \
	...
	or not _require(_capture_time_authority_matches(_capture_time_authority_receipt()), "B225 authority is not exact catalog/registry v8, contracts/loader v7, compiler 1.7.0, 8/213, with one frozen-evidence-bound B225 acceptance and sole current topology ownership.") \
	or not _require(_package_verification_argument_matches(), "B225 postpromotion capture requires the exact fresh, acyclic v8 package-verification receipt and receipt SHA arguments.") \
...
func _validate_production_bindings(world: WorldLoader) -> Dictionary:
	...
	return {"ok": true, "metadata": {
		...
		"production_stage": "postpromotion_v8_current_authority_uncommitted_candidate",
		"canonical_entrypoint_reachable": true, "additional_recognition_credit": false, "postcapture_catalog_or_registry_mutation": false,
		"capture_time_recognition_metric": "8/213",
		"current_v8_acceptance_governed_by_catalog_registry": true,
		"live_node_pending_labels_retained_as_frozen_capture_provenance": true,
...
func _capture_stills_b225(main: GameMain, world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, bindings: Dictionary, output_root: String, authoritative: bool) -> void:
	...
	if not _require(_live_binding_ids_unchanged(world, bindings), "Production live binding identities drifted during still capture."):
		return
	var package_receipt := _package_verification_receipt_from_argument()
	var manifest := {
		"schema_version": "ti.d1-b225-postpromotion-v8-static-evidence/1",
		"review_status": "pending_independent_postpromotion_contract_and_visual_audits_uncommitted_candidate_not_self_accepted",
		"recognition_status": "current_v8_accepted_no_additional_credit_pending_two_postpromotion_audits",
		"capture_time_recognition_metric": "8/213",
		"production_stage": "postpromotion_v8_current_authority_uncommitted_candidate",
		"postpromotion_candidate_evidence": authoritative,
		"uncommitted_candidate": true,
		"pending_independent_postpromotion_contract_audit": true,
		"pending_independent_postpromotion_visual_review": true,
		...
		"candidate_generator_sha256_argument": _argument_value("--generator-sha256="),
		...
		"package_verification_receipt": package_receipt,
		...
		"canonical_entrypoint_reachable": true, "postcapture_catalog_or_registry_mutation": false, "additional_recognition_credit": false,
		...
		"visual_verdict": "pending_independent_postpromotion_review",
		"nonclaims": ["additional recognition credit beyond the already accepted v8 numerator", "perfect texture tiling", "exact pane count, cadence, damage, lower openings, entries, other sides, roof equipment, interior, or as-built realism"],
	}
	...
	if not _copy_package_verification_receipt(output_root):
		_fail("Could not copy the exact package-verification receipt into the B225 packet.")
		return
	if not _sealed_static_stage_matches(output_root, "static", authoritative):
		...
		return
	print("PASS: captured eleven fresh B225 production-path stills at 1440x900 under current v8 8/213 authority with no additional recognition credit")
...
func _capture_motion_telemetry_b225(world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, bindings: Dictionary, output_root: String, authoritative: bool) -> void:
	...
	var package_receipt := _package_verification_receipt_from_argument()
	var manifest := {
		"schema_version": "ti.d1-b225-postpromotion-v8-motion-telemetry/1",
		"review_status": "pending_independent_postpromotion_contract_and_visual_audits_uncommitted_candidate_not_self_accepted",
		"recognition_status": "current_v8_accepted_no_additional_credit_pending_two_postpromotion_audits", "capture_time_recognition_metric": "8/213",
		"production_stage": "postpromotion_v8_current_authority_uncommitted_candidate",
		"postpromotion_candidate_evidence": authoritative,
		"uncommitted_candidate": true,
		"pending_independent_postpromotion_contract_audit": true,
...
func _capture_visual_motion_b225(world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, bindings: Dictionary, output_root: String, authoritative: bool) -> void:
	...
	var package_receipt := _package_verification_receipt_from_argument()
	var manifest := {
		"schema_version": "ti.d1-b225-postpromotion-v8-visual-motion/1",
		"review_status": "pending_independent_postpromotion_contract_and_visual_audits_uncommitted_candidate_not_self_accepted",
		"recognition_status": "current_v8_accepted_no_additional_credit_pending_two_postpromotion_audits", "capture_time_recognition_metric": "8/213",
		"production_stage": "postpromotion_v8_current_authority_uncommitted_candidate",
		...
		"recovery_delta": 0, "visual_verdict": "pending_independent_postpromotion_review",
		"canonical_entrypoint_reachable": true, "postcapture_catalog_or_registry_mutation": false, "additional_recognition_credit": false,
	}
...
func _save_production_still(main: GameMain, player: PlayerController, hud: GameHUD, output_root: String, id: String, role: String, lighting: String, extra: Dictionary) -> Dictionary:
	...
	var metadata := {"id": id, "role": role, "lighting": lighting, "file": relative, "sha256": FileAccess.get_sha256(path), "bytes": FileAccess.get_file_as_bytes(path).size(), "dimensions": [image.get_width(), image.get_height()], "sample_unique_colors": int(sample.unique_colors), "sample_luminance_range": float(sample.luminance_range), "debug_labels_visible": false, "source_photography_in_frame": false, "crop_or_postprocess": false, "production_stage": "postpromotion_v8_current_authority_uncommitted_candidate", "visual_verdict": "pending_independent_postpromotion_review"}
...
func _save_grayscale_b225(output_root: String, source_relative: String, target_relative: String) -> Dictionary:
	...
	return {"ok": true, "metadata": {"id": "04-whole-nnw-default-grayscale", "role": "lossless_grayscale_legibility_derivative", "lighting": "derived_from_world_default", "file": target_relative, "sha256": FileAccess.get_sha256(target_path), "bytes": FileAccess.get_file_as_bytes(target_path).size(), "dimensions": [image.get_width(), image.get_height()], "derivation_source": source_relative, "derivation_source_sha256": FileAccess.get_sha256(source_path), "crop_or_postprocess": false, "grayscale_only": true, "visual_verdict": "pending_independent_postpromotion_review"}}
...
func _postpromotion_manifest_claims_match(manifest: Dictionary, authoritative: bool) -> bool:
	return _json_bool_equals(manifest.get("postpromotion_candidate_evidence"), authoritative) \
		and _json_bool_equals(manifest.get("uncommitted_candidate"), true) \
		and _json_bool_equals(manifest.get("pending_independent_postpromotion_contract_audit"), true) \
		and _json_bool_equals(manifest.get("pending_independent_postpromotion_visual_review"), true) \
		and str(manifest.get("review_status", "")) == "pending_independent_postpromotion_contract_and_visual_audits_uncommitted_candidate_not_self_accepted" \
		and str(manifest.get("recognition_status", "")) == "current_v8_accepted_no_additional_credit_pending_two_postpromotion_audits" \
		and str(manifest.get("capture_time_recognition_metric", "")) == "8/213" \
		and str(manifest.get("production_stage", "")) == "postpromotion_v8_current_authority_uncommitted_candidate" \
		and str(manifest.get("visual_verdict", "")) == "pending_independent_postpromotion_review" \
		and _json_bool_equals(manifest.get("postcapture_catalog_or_registry_mutation"), false) \
		and _json_bool_equals(manifest.get("additional_recognition_credit"), false)
...
func _sealed_static_stage_matches(output_root: String, stage: String, authoritative: bool, manifest_override: Variant = null, run_mutations: bool = true) -> bool:
	...
	if str(manifest.get("schema_version", "")) != "ti.d1-b225-postpromotion-v8-static-evidence/1" \
	or str(manifest.get("generator_sha256", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	or str(manifest.get("candidate_generator_sha256_argument", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	...
	or not _json_bool_equals(manifest.get("postpromotion_candidate_evidence"), authoritative) \
	or not _postpromotion_manifest_claims_match(manifest, authoritative) \
	or str(manifest.get("production_stage", "")) != "postpromotion_v8_current_authority_uncommitted_candidate" \
	or str(manifest.get("capture_time_recognition_metric", "")) != "8/213" \
	or str(manifest.get("recognition_status", "")) != "current_v8_accepted_no_additional_credit_pending_two_postpromotion_audits" \
	...
	or not _package_verification_packet_receipt_matches(output_root, manifest) \
	...
	or not _json_bool_equals(manifest.get("postcapture_catalog_or_registry_mutation"), false) \
	or not _json_bool_equals(manifest.get("additional_recognition_credit"), false) \
...
func _static_disk_numeric_mutations_reject(output_root: String, stage: String, authoritative: bool, manifest: Dictionary) -> bool:
	...
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("capture_time_authority", {}) as Dictionary)["runtime_registry_schema_version"] = "ti.facade-runtime-registry/7"
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("package_verification_receipt", {}) as Dictionary)["pck_bytes"] = float((mutation.get("package_verification_receipt", {}) as Dictionary).get("pck_bytes", 0)) + 0.5
	mutations.append(mutation)
...
func _sealed_telemetry_stage_matches(output_root: String, stage: String, authoritative: bool, manifest_override: Variant = null, run_mutations: bool = true) -> bool:
	...
	if not manifest_value is Dictionary:
		return false
	var manifest := manifest_value as Dictionary
	if str(manifest.get("schema_version", "")) != "ti.d1-b225-postpromotion-v8-motion-telemetry/1" \
	or str(manifest.get("generator_sha256", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	or str(manifest.get("candidate_generator_sha256_argument", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	or str(manifest.get("capture_override_sha256", "")) != CAPTURE_OVERRIDE_SHA256 \
...
func _telemetry_disk_numeric_mutations_reject(output_root: String, stage: String, authoritative: bool, manifest: Dictionary) -> bool:
	...
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("capture_time_authority", {}) as Dictionary)["recognition_metric_numerator"] = 9
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("package_verification_receipt", {}) as Dictionary)["additional_recognition_credit"] = true
	mutations.append(mutation)
	for mutated: Dictionary in mutations:
		var parsed_mutation: Variant = JSON.parse_string(JSON.stringify(mutated))
		if not parsed_mutation is Dictionary \
...
func _final_visual_stage_matches(output_root: String, authoritative: bool, manifest_name: String = "visual-motion.json", manifest_override: Variant = null, run_mutations: bool = true) -> bool:
	...
	if not manifest_value is Dictionary:
		return false
	var manifest := manifest_value as Dictionary
	if str(manifest.get("schema_version", "")) != "ti.d1-b225-postpromotion-v8-visual-motion/1" \
	or str(manifest.get("generator_sha256", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	or str(manifest.get("candidate_generator_sha256_argument", "")) != FileAccess.get_sha256(B225_GENERATOR) \
	or str(manifest.get("capture_override_sha256", "")) != CAPTURE_OVERRIDE_SHA256 \
	or not _json_bool_equals(manifest.get("postpromotion_candidate_evidence"), authoritative) \
...
func _visual_disk_numeric_mutations_reject(output_root: String, authoritative: bool, manifest_name: String, manifest: Dictionary) -> bool:
	...
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("capture_time_authority", {}) as Dictionary)["b225_catalog_acceptance_record_count"] = 2
	mutations.append(mutation)
	mutation = manifest.duplicate(true)
	(mutation.get("package_verification_receipt", {}) as Dictionary)["generator_sha256"] = "0".repeat(64)
	mutations.append(mutation)
	for mutated: Dictionary in mutations:
		var parsed_mutation: Variant = JSON.parse_string(JSON.stringify(mutated))
		if not parsed_mutation is Dictionary \
...
func _capture_time_authority_receipt() -> Dictionary:
	...
	var registry_counts := registry.get("counts", {}) as Dictionary
	var catalog_counts := catalog.get("expected_counts", {}) as Dictionary
	var claim_totals := registry.get("claim_totals", {}) as Dictionary
	var reference_claims := claim_totals.get("reference_recognizable", {}) as Dictionary
	var contract_counts := contracts.get("counts", {}) as Dictionary
	var registry_unit := _authority_unit(registry.get("units", []) as Array, B225_UNIT_ID)
	var catalog_unit := _authority_unit(catalog.get("units", []) as Array, B225_UNIT_ID)
	...
	var registry_receipt := registry_receipts[0] as Dictionary if registry_receipts.size() == 1 and registry_receipts[0] is Dictionary else {}
	var catalog_receipt := catalog_receipts[0] as Dictionary if catalog_receipts.size() == 1 and catalog_receipts[0] is Dictionary else {}
	var b225_adapter := _authority_adapter_for_receiver(registry.get("active_runtime_adapters", []) as Array, RECEIVER_KEY)
	var b201_adapter := _authority_adapter_for_receiver(registry.get("active_runtime_adapters", []) as Array, B201_RECEIVER_KEY)
	var b225_behavior := _adapter_behavior(b225_adapter)
	var b201_behavior := _adapter_behavior(b201_adapter)
	...
	for value: Variant in registry.get("active_runtime_adapters", []) as Array:
		...
		var geometry := _adapter_behavior(adapter).get("geometry_contract", {}) as Dictionary
	...
	current_topology_owners.sort()
	var authority_text := FileAccess.get_file_as_string(REGISTRY_PATH) + "\n" + FileAccess.get_file_as_string(CATALOG_PATH) + "\n" + FileAccess.get_file_as_string(ADAPTER_CONTRACTS_PATH)
	return {
		"schema_version": "ti.d1-b225-postpromotion-v8-authority-snapshot/1",
		...
		"recognized_physical_unit_count": (metric.get("accepted_physical_unit_ids", []) as Array).size(),
		"not_evaluated_physical_unit_count": reference_claims.get("not_evaluated", -1),
		"registry_unit_count": (registry.get("units", []) as Array).size(),
		"direct_wall_receiver_count": registry_counts.get("direct_wall_receivers", -1),
		"source_record_membership_count": registry_counts.get("source_record_memberships", -1),
		"visible_wall_run_count": registry_counts.get("visible_wall_runs", -1),
		...
		"total_runtime_adapter_receiver_count": catalog_counts.get("runtime_adapter_receivers", -1),
		"package_safe_adapter_plan_count": contract_counts.get("package_safe_adapter_receivers", -1),
		"disabled_adapter_plan_count": contract_counts.get("disabled_adapter_receivers", -1),
		"b225_physical_unit_id": B225_UNIT_ID,
		"b225_catalog_reference_recognizable": str((catalog_unit.get("claim_status", {}) as Dictionary).get("reference_recognizable", "")),
		"b225_catalog_acceptance_record_count": catalog_receipts.size(),
		...
		"b225_registry_acceptance_record_count": registry_receipts.size(),
		"b225_registry_active_runtime_adapter_ids": (registry_unit.get("active_runtime_adapter_ids", []) as Array).duplicate(),
		...
		"b225_registry_receiver_runtime_adapter_id": str(registry_receiver.get("runtime_adapter_id", "")),
		"b225_acceptance_capture_time_recognition_metric": str(registry_receipt.get("capture_time_recognition_metric", "")),
		"b225_acceptance_evidence_manifest_sha256": str(registry_receipt.get("evidence_manifest_sha256", "")),
		"b225_acceptance_evidence_tree_sha256": str(registry_receipt.get("evidence_tree_sha256", "")),
		"b225_acceptance_review_receipt_sha256": str(registry_receipt.get("review_receipt_sha256", "")),
		"b225_acceptance_numerator_effect": registry_receipt.get("numerator_effect", -1),
		"catalog_and_registry_acceptance_receipts_equal": registry_receipt == catalog_receipt,
		"b225_adapter_acceptance_contract_matches": str(b225_acceptance.get("accepted_physical_unit_id", "")) == B225_UNIT_ID and int(b225_acceptance.get("numerator_effect", -1)) == 1,
		"b225_current_topology": _geometry_topology_receipt(b225_geometry),
		"b225_current_topology_scope": str(b225_geometry.get("world_topology_scope", "")),
		"b201_pre_b225_topology": _geometry_topology_receipt(b201_geometry),
		"b201_topology_scope": str(b201_geometry.get("world_topology_scope", "")),
		"current_topology_owner_adapter_ids": current_topology_owners,
		"frozen_live_node_pending_labels_retained_as_capture_provenance": true,
		"downstream_postpromotion_hashes_bound_into_v8_authority": authority_text.contains("d1-b225-postpromotion-v8-2026-09-05") or authority_text.contains(PACKAGE_RECEIPT_SCHEMA),
...
func _capture_time_authority_matches(value: Variant) -> bool:
	...
	var expected_keys := [
		...
		"additional_recognition_credit",
		"b201_pre_b225_topology",
		"b201_topology_scope",
		"b225_acceptance_capture_time_recognition_metric",
		"b225_acceptance_evidence_manifest_sha256",
		"b225_acceptance_evidence_tree_sha256",
		"b225_acceptance_numerator_effect",
		"b225_acceptance_review_receipt_sha256",
		"b225_adapter_acceptance_contract_matches",
		"b225_catalog_acceptance_record_count",
		"b225_catalog_active_adapter_record_count",
		"b225_catalog_reference_recognizable",
		"b225_current_topology",
		"b225_current_topology_scope",
		"b225_physical_unit_id",
		"b225_registry_acceptance_record_count",
		"b225_registry_active_adapter_record_count",
		"b225_registry_active_runtime_adapter_ids",
		"b225_registry_receiver_runtime_adapter_id",
		"b225_registry_receiver_runtime_content_mode",
		"b225_registry_reference_recognizable",
		"catalog_active_runtime_adapter_count",
		"catalog_and_registry_acceptance_receipts_equal",
		"catalog_schema_version",
		"compiler_version",
		"current_topology_owner_adapter_ids",
		"direct_wall_receiver_count",
		"disabled_adapter_plan_count",
		"downstream_postpromotion_hashes_bound_into_v8_authority",
		"frozen_live_node_pending_labels_retained_as_capture_provenance",
		"loader_api_version",
		"not_evaluated_physical_unit_count",
		"package_safe_adapter_plan_count",
		"postcapture_catalog_or_registry_mutation",
		"recognition_metric_denominator",
		"recognition_metric_display",
		"recognition_metric_numerator",
		"recognized_physical_unit_count",
		"registry_active_runtime_adapter_count",
		"registry_unit_count",
		"runtime_registry_schema_version",
		"schema_version",
		"source_record_membership_count",
		"total_runtime_adapter_receiver_count",
		"visible_wall_run_count",
	]
	keys.sort()
	expected_keys.sort()
	return keys == expected_keys \
		and str(receipt.get("schema_version", "")) == "ti.d1-b225-postpromotion-v8-authority-snapshot/1" \
		and str(receipt.get("catalog_schema_version", "")) == "ti.facade-recognition-catalog/8" \
		and str(receipt.get("runtime_registry_schema_version", "")) == "ti.facade-runtime-registry/8" \
		and str(receipt.get("adapter_contracts_schema_version", "")) == "ti.facade-runtime-adapter-contracts/7" \
		and str(receipt.get("loader_api_version", "")) == "ti.facade-runtime-registry-loader/7" \
		and str(receipt.get("compiler_version", "")) == "1.7.0" \
		and str(receipt.get("recognition_metric_display", "")) == "8/213" \
		and _json_int_equals(receipt.get("recognition_metric_numerator"), 8) \
		and _json_int_equals(receipt.get("recognition_metric_denominator"), 213) \
		and _json_int_equals(receipt.get("recognized_physical_unit_count"), 8) \
		and _json_int_equals(receipt.get("not_evaluated_physical_unit_count"), 205) \
		and _json_int_equals(receipt.get("registry_unit_count"), 213) \
		and _json_int_equals(receipt.get("direct_wall_receiver_count"), 214) \
		and _json_int_equals(receipt.get("source_record_membership_count"), 215) \
		and _json_int_equals(receipt.get("visible_wall_run_count"), 4971) \
		and _json_int_equals(receipt.get("registry_active_runtime_adapter_count"), 7) \
		and _json_int_equals(receipt.get("catalog_active_runtime_adapter_count"), 7) \
		and _json_int_equals(receipt.get("total_runtime_adapter_receiver_count"), 16) \
		and _json_int_equals(receipt.get("package_safe_adapter_plan_count"), 10) \
		and _json_int_equals(receipt.get("disabled_adapter_plan_count"), 6) \
		and str(receipt.get("b225_physical_unit_id", "")) == B225_UNIT_ID \
		and str(receipt.get("b225_catalog_reference_recognizable", "")) == "accepted" \
		and _json_int_equals(receipt.get("b225_catalog_acceptance_record_count"), 1) \
		and _json_int_equals(receipt.get("b225_catalog_active_adapter_record_count"), 1) \
		and str(receipt.get("b225_registry_reference_recognizable", "")) == "accepted" \
		and _json_int_equals(receipt.get("b225_registry_acceptance_record_count"), 1) \
		and receipt.get("b225_registry_active_runtime_adapter_ids", []) == [B225_ADAPTER_ID] \
		and _json_int_equals(receipt.get("b225_registry_active_adapter_record_count"), 1) \
		and str(receipt.get("b225_registry_receiver_runtime_content_mode", "")) == "active_d1_b225_host_partition_attachment" \
		and str(receipt.get("b225_registry_receiver_runtime_adapter_id", "")) == B225_ADAPTER_ID \
		and str(receipt.get("b225_acceptance_capture_time_recognition_metric", "")) == "7/213" \
		and str(receipt.get("b225_acceptance_evidence_manifest_sha256", "")) == str(DEPENDENCY_HASHES[FROZEN_PRODUCTION_MANIFEST_PATH]) \
		and str(receipt.get("b225_acceptance_evidence_tree_sha256", "")) == FROZEN_PRODUCTION_TREE_SHA256 \
		and str(receipt.get("b225_acceptance_review_receipt_sha256", "")) == str(DEPENDENCY_HASHES[FROZEN_PRODUCTION_REVIEW_PATH]) \
		and _json_int_equals(receipt.get("b225_acceptance_numerator_effect"), 1) \
		and _json_bool_equals(receipt.get("catalog_and_registry_acceptance_receipts_equal"), true) \
		and _json_bool_equals(receipt.get("b225_adapter_acceptance_contract_matches"), true) \
		and _json_topology_matches(receipt.get("b225_current_topology"), PRODUCTION_WORLD_TOPOLOGY) \
		and str(receipt.get("b225_current_topology_scope", "")) == "current_integration_topology" \
		and _json_topology_matches(receipt.get("b201_pre_b225_topology"), PRE_B225_WORLD_TOPOLOGY) \
		and str(receipt.get("b201_topology_scope", "")) == "pre_b225_integration_live_parity" \
		and receipt.get("current_topology_owner_adapter_ids", []) == [B225_ADAPTER_ID] \
		and _json_bool_equals(receipt.get("frozen_live_node_pending_labels_retained_as_capture_provenance"), true) \
		and _json_bool_equals(receipt.get("downstream_postpromotion_hashes_bound_into_v8_authority"), false) \
		and _json_bool_equals(receipt.get("postcapture_catalog_or_registry_mutation"), false) \
		and _json_bool_equals(receipt.get("additional_recognition_credit"), false)


func _authority_adapter_for_receiver(adapters: Array, receiver_key: String) -> Dictionary:
...
func _adapter_behavior(adapter: Dictionary) -> Dictionary:
	var active_value: Variant = adapter.get("active_runtime_contract")
	if not active_value is Dictionary:
		return {}
	var behavior_value: Variant = (active_value as Dictionary).get("behavior_contract")
	return behavior_value as Dictionary if behavior_value is Dictionary else {}


func _geometry_topology_receipt(geometry: Dictionary) -> Dictionary:
	return {
		"rows": geometry.get("world_records", -1),
		"meshes": geometry.get("world_mesh_instances", -1),
		"surfaces": geometry.get("world_surfaces", -1),
		"triangles": geometry.get("world_triangles", -1),
		"bodies": geometry.get("world_static_bodies", -1),
		"shapes": geometry.get("world_shapes", -1),
	}
...
func _package_verification_argument_matches() -> bool:
	var path := _argument_value("--package-verification-receipt=")
	var expected_sha256 := _argument_value("--package-verification-receipt-sha256=")
	...
	or not FileAccess.file_exists(path) or FileAccess.get_sha256(path) != expected_sha256:
		return false
	return _package_verification_receipt_matches(_package_verification_receipt_from_argument())


func _package_verification_receipt_from_argument() -> Dictionary:
	var path := _argument_value("--package-verification-receipt=")
	var expected_sha256 := _argument_value("--package-verification-receipt-sha256=")
	if not path.is_absolute_path() or not _lowercase_sha256(expected_sha256) \
	or not FileAccess.file_exists(path) or FileAccess.get_sha256(path) != expected_sha256:
...
func _package_verification_packet_receipt_matches(output_root: String, manifest: Dictionary) -> bool:
	var copied_path := output_root.path_join(PACKAGE_RECEIPT_FILENAME)
	var receipt_sha256 := str(manifest.get("package_verification_receipt_sha256", ""))
	if not _json_bool_equals(manifest.get("package_verification_receipt_copied_into_packet"), true) \
	or not _lowercase_sha256(receipt_sha256) \
	or receipt_sha256 != _argument_value("--package-verification-receipt-sha256=") \
	...
	var embedded_value: Variant = manifest.get("package_verification_receipt")
	if not copied_value is Dictionary or not embedded_value is Dictionary \
	or not _package_verification_receipt_matches(copied_value) \
	or not _package_verification_receipt_matches(embedded_value):
		return false
	return _package_verification_receipts_same(copied_value as Dictionary, embedded_value as Dictionary)


func _package_verification_receipt_matches(value: Variant) -> bool:
	if not value is Dictionary:
		return false
	var receipt := value as Dictionary
	...
	var expected_keys := [
		...
		"bundle_inventory",
		"bundle_verification_log_sha256",
		"candidate_status",
		"catalog_schema_version",
		"capture_date",
		"capture_time_recognition_metric",
		"codesign_deep_strict_pass",
		"codesign_per_slice_pass",
		"compiler_version",
		"downstream_postpromotion_hashes_bound_into_v8_authority",
		"entitlements_empty_dictionary",
		"executable_bytes",
		"executable_sha256",
		"extended_attributes_absent",
		...
		"mounted_new_v8_package_contract_present_and_loadable",
		...
		"source_hashes",
		"source_package_contract_log_sha256",
		"source_package_contract_pass",
	]
	actual_keys.sort()
	expected_keys.sort()
	var six := receipt.get("six_production_asset_hashes", {}) as Dictionary
	var source_hashes := receipt.get("source_hashes", {}) as Dictionary
	return actual_keys == expected_keys \
		...
		and str(receipt.get("candidate_status", "")) == "uncommitted_v8_candidate_pending_two_independent_postpromotion_audits" \
		and str(receipt.get("review_status", "")) == "pending_independent_postpromotion_contract_and_visual_audits_not_self_accepted" \
		and str(receipt.get("production_stage", "")) == "postpromotion_v8_current_authority_uncommitted_candidate" \
		and str(receipt.get("capture_time_recognition_metric", "")) == "8/213" \
		and str(receipt.get("catalog_schema_version", "")) == "ti.facade-recognition-catalog/8" \
		and str(receipt.get("runtime_registry_schema_version", "")) == "ti.facade-runtime-registry/8" \
		and str(receipt.get("adapter_contracts_schema_version", "")) == "ti.facade-runtime-adapter-contracts/7" \
		and str(receipt.get("loader_api_version", "")) == "ti.facade-runtime-registry-loader/7" \
		and str(receipt.get("compiler_version", "")) == "1.7.0" \
		and str(receipt.get("app_bundle_name", "")) == "Treasure Island First Playable.app" \
		and _json_int_equals(receipt.get("bundle_file_count"), 7) \
		and str(receipt.get("bundle_identity_algorithm", "")) == "ascii_relative_path_nul_sha256_nul_bytes_lf_sorted_lc_all_c" \
		and _lowercase_sha256(str(receipt.get("bundle_identity_sha256", ""))) \
		and _bundle_inventory_matches(receipt) \
		and source_hashes == _expected_source_hashes() \
		...
		and _lowercase_sha256(str(receipt.get("executable_sha256", ""))) \
		and _json_integral_number(receipt.get("executable_bytes")) and int(receipt.executable_bytes) > 0 \
		and receipt.get("architectures", []) == ["x86_64", "arm64"] \
		and _lowercase_sha256(str(receipt.get("bundle_verification_log_sha256", ""))) \
		and _lowercase_sha256(str(receipt.get("source_package_contract_log_sha256", ""))) \
		...
		and _json_bool_equals(receipt.get("mounted_new_v8_package_contract_present_and_loadable"), true) \
		and _json_bool_equals(receipt.get("codesign_deep_strict_pass"), true) \
		and _json_bool_equals(receipt.get("codesign_per_slice_pass"), true) \
		and _json_bool_equals(receipt.get("entitlements_empty_dictionary"), true) \
		and _json_bool_equals(receipt.get("privacy_manifest_present"), true) \
		and _json_bool_equals(receipt.get("privacy_manifest_valid"), true) \
		and _json_bool_equals(receipt.get("extended_attributes_absent"), true) \
		and _json_bool_equals(receipt.get("quarantine_absent"), true) \
...
func _package_verification_receipts_same(first: Dictionary, second: Dictionary) -> bool:
	return _package_verification_receipt_matches(first) \
		and _package_verification_receipt_matches(second) \
		and first == second


func _bundle_inventory_matches(receipt: Dictionary) -> bool:
	var inventory_value: Variant = receipt.get("bundle_inventory")
	if not inventory_value is Array:
		return false
	var inventory := inventory_value as Array
	var expected_paths := [
		"Contents/Info.plist",
	...
	if inventory.size() != expected_paths.size():
		return false
	...
	for index in inventory.size():
		...
		var item_keys := Array(item.keys())
		item_keys.sort()
		if item_keys != ["bytes", "path", "sha256"]:
			return false
		var path := str(item.get("path", ""))
		var sha256 := str(item.get("sha256", ""))
		if path != expected_paths[index] or path.begins_with("/") \
		or path.to_ascii_buffer().get_string_from_ascii() != path \
		or not _lowercase_sha256(sha256) \
		or not _json_integral_number(item.get("bytes")) or int(item.bytes) <= 0:
			return false
		canonical.append_array(path.to_ascii_buffer())
		canonical.append(0)
		canonical.append_array(sha256.to_ascii_buffer())
		canonical.append(0)
		canonical.append_array((str(int(item.bytes)) + "\n").to_ascii_buffer())
	var executable := inventory[1] as Dictionary
	var pck := inventory[4] as Dictionary
	var hashing := HashingContext.new()
	if hashing.start(HashingContext.HASH_SHA256) != OK or hashing.update(canonical) != OK:
		return false
	var canonical_sha256 := hashing.finish().hex_encode()
	return str(executable.sha256) == str(receipt.get("executable_sha256", "")) \
		and _json_int_equals(receipt.get("executable_bytes"), int(executable.bytes)) \
		and str(pck.sha256) == str(receipt.get("pck_sha256", "")) \
		and _json_int_equals(receipt.get("pck_bytes"), int(pck.bytes)) \
		and canonical_sha256 == str(receipt.get("bundle_identity_sha256", ""))


func _expected_source_hashes() -> Dictionary:
	return {
		"project.godot": str(DEPENDENCY_HASHES[PROJECT_SETTINGS_PATH]),
		"game/scripts/main.gd": str(DEPENDENCY_HASHES["res://game/scripts/main.gd"]),
		"game/scripts/world/world_chunk_builder.gd": str(DEPENDENCY_HASHES[B225_BUILDER_PATH]),
		"game/scripts/world/facades/d1_b225_live_attachment.gd": str(DEPENDENCY_HASHES[ADAPTER_PATH]),
		"game/resources/facades/d1_current/d1_b225_live_attachment.json": str(DEPENDENCY_HASHES[CONFIG_PATH]),
		"discovery/facades/facade-recognition-catalog.json": str(DEPENDENCY_HASHES[CATALOG_PATH]),
		"discovery/facades/facade-recognition-catalog.schema.json": str(DEPENDENCY_HASHES[CATALOG_SCHEMA_PATH]),
		"game/resources/facades/facade-runtime-registry.json": str(DEPENDENCY_HASHES[REGISTRY_PATH]),
		"game/resources/facades/facade-runtime-adapter-contracts.json": str(DEPENDENCY_HASHES[ADAPTER_CONTRACTS_PATH]),
		"game/scripts/world/facades/facade_runtime_registry_loader.gd": str(DEPENDENCY_HASHES[RUNTIME_LOADER_PATH]),
		"game/tests/headless_d1_b225_postpromotion_v8_contract.gd": str(DEPENDENCY_HASHES[FOCUSED_CONTRACT_PATH]),
		"game/tests/headless_d1_b225_postpromotion_v8_package_contract.gd": str(DEPENDENCY_HASHES[PACKAGE_CONTRACT_PATH]),
		"game/tests/d1_b225_postpromotion_v8_capture.gd": FileAccess.get_sha256(B225_GENERATOR),
		"export_presets.cfg": str(DEPENDENCY_HASHES[EXPORT_PRESET_PATH]),
		"evidence/first-playable/d1-b225-production-attachment-v7-staging-2026-09-05/capture-manifest.json": str(DEPENDENCY_HASHES[FROZEN_PRODUCTION_MANIFEST_PATH]),
		"evidence/first-playable/d1-b225-production-attachment-v7-staging-2026-09-05/tree_sha256": FROZEN_PRODUCTION_TREE_SHA256,
		"evidence/reviews/d1-b225-production-attachment-v7-staging-2026-09-05-INDEPENDENT_VISUAL_REVIEW.md": str(DEPENDENCY_HASHES[FROZEN_PRODUCTION_REVIEW_PATH]),
	}
...
func _bindings_receipt_matches(value: Variant) -> bool:
	...
	or str(receipt.get("production_stage", "")) != "postpromotion_v8_current_authority_uncommitted_candidate" \
	or str(receipt.get("receiver_key", "")) != RECEIVER_KEY \
	or str(receipt.get("roof_key", "")) != ROOF_KEY \
	or receipt.get("source_keys", []) != [SOURCE_KEY] \
	or str(receipt.get("capture_time_recognition_metric", "")) != "8/213" \
	or not _json_bool_equals(receipt.get("canonical_entrypoint_reachable"), true) \
	or not _json_bool_equals(receipt.get("additional_recognition_credit"), false) \
	or not _json_bool_equals(receipt.get("postcapture_catalog_or_registry_mutation"), false) \
	or not _json_bool_equals(receipt.get("current_v8_acceptance_governed_by_catalog_registry"), true) \
	or not _json_bool_equals(receipt.get("live_node_pending_labels_retained_as_frozen_capture_provenance"), true) \
...
func _copy_package_verification_receipt(output_root: String) -> bool:
	var source := _argument_value("--package-verification-receipt=")
	var expected_sha256 := _argument_value("--package-verification-receipt-sha256=")
	var target := output_root.path_join(PACKAGE_RECEIPT_FILENAME)
	if not _package_verification_argument_matches() or FileAccess.file_exists(target):
		return false
	var file := FileAccess.open(target, FileAccess.WRITE)
	if file == null:
		return false
