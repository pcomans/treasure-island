...
const MOTION_FILE := "isle-house-composite-repair-public-corner-stock-walk.avi"
...
func _run() -> void:
	...
	if DisplayServer.get_name() == "headless":
		_fail("Standalone composite evidence requires native macOS Forward+/Metal.")
		_finish(null)
		return
	...
	if not _require(bool(binding_result.get("ok", false)), str(binding_result.get("message", "Standalone preview binding failed."))):
		_finish(main)
		return
	...
	if not _require(DirAccess.make_dir_recursive_absolute(output_absolute.path_join("images")) == OK, "Could not create standalone composite evidence directory."):
		_finish(main)
		return
	paused = false
...
func _attach_standalone_preview(world: WorldLoader) -> Dictionary:
	...
	if packed == null:
		return {"ok": false, "message": "Standalone composite scene did not load."}
	var standalone := packed.instantiate() as IsleHouseCompositeRepairStandaloneV1
	var configured := standalone.configure_from_chunk()
	if not bool(configured.get("ok", false)) or str(configured.get("repair_signature", "")) != REPAIR_SIGNATURE:
		standalone.free()
		return {"ok": false, "message": "Standalone composite factory failed: %s" % configured}
	var candidate := standalone.detach_repair_overlay_for_preview()
	if candidate == null:
		standalone.free()
		return {"ok": false, "message": "Standalone factory did not expose its reversible preview seam."}
	standalone.free()
	candidate.name = "StandaloneCompositeRepairPreview"
	low.add_child(candidate)
	candidate.hide()
	live.show()
	...
	or _count_type(candidate, CollisionObject3D) != 0 \
	or _count_type(candidate, CollisionShape3D) != 0 \
	or _count_type(candidate, NavigationRegion3D) != 0 \
	or _any_node_in_group(candidate, "spray_receiver") \
	or _any_node_in_group(candidate, "spray_receiver_wall"):
		return {"ok": false, "message": "Detached candidate lost exact records or acquired forbidden ownership."}
	return {
		"ok": true,
		"low": low,
		"high": high,
		...
		"metadata": {
			...
			"candidate_topology": candidate.get_meta("topology", {}).duplicate(true),
...
func _set_preview(bindings: Dictionary, variant: String) -> bool:
	...
	var candidate := bindings.candidate as Node3D
	if variant == "failed_parent":
		live.show()
		candidate.hide()
		return true
	if variant == "repair":
...
func _capture_stills(main: GameMain, world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, bindings: Dictionary) -> void:
	...
	for view_value: Variant in WHOLE_VIEWS:
		...
		for variant: String in ["failed_parent", "repair"]:
			...
			var id := "%s-%s" % [str(view.id), "a-failed-parent" if variant == "failed_parent" else "b-repair"]
			var saved := await _save_still(main, player, hud, id, str(view.side), "same_pose_whole_high_low_A_B", variant, "world_default", extra)
		...
		var changed_id := "%s-b-repair-changed-light" % str(view.id)
		var changed := await _save_still(main, player, hud, changed_id, str(view.side), "same_pose_repair_changed_light", "repair", "frozen_changed_light", changed_extra)
	...
	for view_value: Variant in SUPPLEMENTAL_DETAIL_VIEWS:
		...
		for variant: String in ["failed_parent", "repair"]:
			...
			extra["pair_camera_transform_equal"] = player.get_camera().global_transform.is_equal_approx(stable_transform)
			var id := "%s-%s" % [str(view.id), "a-failed-parent" if variant == "failed_parent" else "b-repair"]
			var saved := await _save_still(main, player, hud, id, str(view.side), "same_pose_ENE_low_detail_supplement", variant, "world_default", extra)
			if not _require(bool(saved.get("ok", false)), str(saved.get("message", "%s save failed." % id))):
				return
			captures.append(saved.metadata as Dictionary)
		_set_preview(bindings, "repair")
		...
		changed_extra["pair_camera_transform_equal"] = player.get_camera().global_transform.is_equal_approx(stable_transform)
		var changed_id := "%s-b-repair-changed-light" % str(view.id)
		var changed := await _save_still(main, player, hud, changed_id, str(view.side), "same_pose_ENE_low_detail_changed_light_supplement", "repair", "frozen_changed_light", changed_extra)
		if not _require(bool(changed.get("ok", false)), str(changed.get("message", "%s save failed." % changed_id))):
			return
		captures.append(changed.metadata as Dictionary)
	sun.rotation_degrees = original_sun
	_set_preview(bindings, "failed_parent")
	var runtime_before_preview: Variant = bindings.runtime_before_preview
	var manifest := {
		"schema_version": "ti.isle-house-composite-repair-standalone-evidence/1",
		...
		"evidence_role": "Native grounded ordinary-player same-pose full-composite A/B plus repair changed-light evidence, with an explicitly supplemental ENE low-detail pair; never a live promotion or executor visual grade.",
		"generator": "game/tests/isle_house_composite_repair_standalone_v1_capture.gd",
		"generator_sha256": FileAccess.get_sha256("res://game/tests/isle_house_composite_repair_standalone_v1_capture.gd"),
		"capture_command": ".tools/godot/4.7.2/Godot.app/Contents/MacOS/Godot --path . --display-driver macos --rendering-method forward_plus --rendering-driver metal --audio-driver Dummy --resolution 1440x900 --script game/tests/isle_house_composite_repair_standalone_v1_capture.gd -- --capture-mode=stills",
		...
		"detached_preview_delta": {"meshes": 6, "surfaces": 8, "triangles": 2316, "bodies": 0, "shapes": 0},
		"captures": captures,
		"capture_count": captures.size(),
		"same_pose_pairs": 3,
		"whole_composite_same_pose_pairs": 2,
		"supplemental_ENE_low_detail_pairs": 1,
		"changed_light_frames": 3,
		...
		"provenance_and_nonclaims": {
			...
			"production_inference": ["four arrangements", "eight placements", "mirroring", "widths", "spacing", "sequence", "upper-field split", "transition reveal depth", "opaque glazing", "planter proxies"],
			"not_claimed": ["literal correction of five generated levels to seven stories", "upper opening or story schedule", "NNW/private/rear/courtyard facade", "roof or hidden interface", "operability", "interiors", "as-built fidelity", "recognition", "believability", "live promotion"],
		},
	}
	if not _write_json(OUTPUT.path_join("capture-manifest.json"), manifest):
		_fail("Could not write standalone composite still manifest.")
		return
	print("PASS: captured %d native grounded same-pose Isle House composite A/B and changed-light stills; repair remains standalone and pending independent review" % captures.size())
...
func _capture_motion(main: GameMain, world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, bindings: Dictionary) -> void:
	...
	var pose := await _settle_and_aim(world, player, hud, requested, target, "standalone-repair-public-corner-motion-start")
	if not _require(bool(pose.get("ok", false)), str(pose.get("message", "Motion start pose failed."))):
		return
	var los := _center_los(player, target, LOW_WALL_KEY, ["w1282547787"], "standalone-repair-public-corner-motion-start")
	...
	or not _require(start_camera.basis.is_equal_approx(player.get_camera().global_transform.basis), "Camera orientation changed during the stock walk."):
		return
	var manifest := {
		"schema_version": "ti.isle-house-composite-repair-standalone-motion-evidence/1",
		"review_status": "pending_independent_review_not_self_accepted",
		"recognition_status": "pending_independent_review",
		"believability_status": "pending_independent_review",
		"evidence_role": "Continuous stock-player walk across repaired WSW run 2/3 join for transition, module-family, shimmer, seam, and stability review; detached preview only.",
		"generator": "game/tests/isle_house_composite_repair_standalone_v1_capture.gd",
		"generator_sha256": FileAccess.get_sha256("res://game/tests/isle_house_composite_repair_standalone_v1_capture.gd"),
		"capture_command": ".tools/godot/4.7.2/Godot.app/Contents/MacOS/Godot --path . --display-driver macos --rendering-method forward_plus --rendering-driver metal --audio-driver Dummy --resolution 1280x800 --write-movie evidence/first-playable/isle-house-composite-repair-standalone-v1-2026-09-04/%s --fixed-fps 30 --script game/tests/isle_house_composite_repair_standalone_v1_capture.gd -- --capture-mode=motion" % MOTION_FILE,
		"godot_version": Engine.get_version_info().get("string", "unknown"),
		"display_driver": DisplayServer.get_name(),
		"video_adapter": RenderingServer.get_video_adapter_name(),
		"rendering_method": "Forward+",
		...
		"architecture_under_review": "four inferred module arrangements plus homogeneous upper podium field and top transition reveal",
	...
	if not _write_json(OUTPUT.path_join("motion-capture.json"), manifest):
		_fail("Could not write standalone composite preliminary motion manifest.")
		return
	print("PASS: drove a %.3f m, %.1f s grounded stock-player walk across the standalone repair with zero transform writes and recoveries; movie finalization remains post-close" % [displacement, float(MOTION_PHYSICS_FRAMES) / MOTION_PHYSICS_HZ])
...
func _finalize_motion_manifest() -> void:
	...
	if not _write_json(manifest_path, manifest):
		_fail("Could not finalize standalone composite motion manifest.")
		return
	print("PASS: finalized standalone composite movie sha256=%s bytes=%d" % [expected_hash, expected_bytes])
