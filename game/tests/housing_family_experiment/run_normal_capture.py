...
import sys,subprocess,json,time,datetime,os,re,zipfile
...
try:
 ...
 reuse=next((arg.split('=',1)[1] for arg in sys.argv[3:] if arg.startswith('--reuse-contact-run=')),None)
 if reuse:
  assert re.fullmatch(r'live-integration-\d+',reuse),reuse
  prior=ROOT/'evidence/housing-family-experiment'/reuse
  partial=json.loads((prior/'images/capture-receipt.json').read_text())
  cases=[x for x in partial['checks'] if 'source' in x]
  targets={x['source_key'] for x in json.loads((ROOT/'game/resources/housing_family/live_instances.json').read_text())['instances']}
  assert len(cases)==24 and {x['source'] for x in cases}==targets and all(x['failures']==0 and x['samples']>0 for x in cases),'Incomplete prior contact subset'
  with zipfile.ZipFile(prior/'rendered-inputs.zip') as archive:
   for path in archive.namelist():
    if path.startswith('game/resources/') or path.startswith('game/scripts/'):
     assert archive.read(path)==(ROOT/path).read_bytes(),'Contact-input drift: '+path
  print('Reusing completed contact subset from '+reuse+'; its overall failed result remains failed.',flush=True)
 cmd=[str(GODOT),'--path',str(ROOT),'--rendering-method','forward_plus','--rendering-driver','metal','--display-driver','macos','--audio-driver','Dummy','--resolution','1440x900','--script',str(ROOT/'game/tests/housing_family_experiment/normal_capture.gd'),'--','--manifest='+str(manifest),'--output='+str(out)]
 if '--mechanics-only' in sys.argv[3:]: cmd.append('--mechanics-only')
 if reuse: cmd.append('--reuse-contact-run='+reuse)
