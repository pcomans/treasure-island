...
const PROTOTYPE_SCENE_PATH := "res://game/scenes/world/facades/site_12_housing/d2_1444_croaker_standalone_hero_prototype.tscn"
...
const MOTION_FILE := "d2-1444-croaker-wsw-stock-walk.avi"
const TARGET_POINT := Vector3(-350.891, 5.45, -38.683)
const PUBLIC_TANGENT := Vector3(-0.435, 0.0, -0.901)
const PUBLIC_OUTWARD := Vector3(0.901, 0.0, -0.435)
const CONFUSION_SET := ["w95934135", "w95934137", "w95934120"]
const CONFUSION_SET_NAMED := [
	{"name": "1431 Halibut Court", "source_key": "w95934135"},
	{"name": "1436 Chinook Court", "source_key": "w95934137"},
	{"name": "1449 Croaker Court", "source_key": "w95934120"},
]
const FROZEN_SUPPORTED_CUES := [
	"broad mostly blank upper end block over a deep opaque exterior undercroft proxy on runs 8 and 9",
	"adjacent WSW return with sparse paired rectangular openings rather than a continuous bay strip",
	"narrow far-end recessed two-level balcony stack plus a shallow dark roof and broad WSW eaves",
]
const VIEWS := [
	{
		"id": "01-whole-wsw",
		"role": "whole_object_grounded_public_wsw_view",
		"requested_xz": Vector2(-316.7, -55.2),
		"aim_target": TARGET_POINT,
		"expected_run_index": 8,
		"sun": DEFAULT_SUN,
	},
	{
		"id": "02-wsw-oblique",
		"role": "grounded_public_wsw_oblique_approach",
		"requested_xz": Vector2(-327.5, -68.0),
		"aim_target": Vector3(-353.371, 5.35, -43.820),
		"expected_run_index": 9,
		"sun": DEFAULT_SUN,
	},
	{
		"id": "03-wsw-oblique-changed-light",
		"role": "same_grounded_public_wsw_oblique_under_materially_changed_light",
		"requested_xz": Vector2(-327.5, -68.0),
		"aim_target": Vector3(-353.371, 5.35, -43.820),
		"expected_run_index": 9,
		"sun": CHANGED_SUN,
...
func _run() -> void:
	...
	if DisplayServer.get_name() == "headless":
		_fail("1444 Croaker evidence requires native macOS Forward+/Metal.")
		_finish(null)
		return
	if not _require(FileAccess.get_sha256(CHUNK_PATH) == EXPECTED_CHUNK_SHA256, "Frozen source chunk drifted before capture.") \
	or not _require(FileAccess.get_sha256(PACKET_PATH) == EXPECTED_PACKET_SHA256, "Authoritative 1444 packet drifted before capture."):
		_finish(null)
		return
	...
	if not _require(PROTOTYPE.matches_record_pair(wall_record, roof_record), "Exact 1444 wall+roof pair failed the standalone seam."):
		_finish(null)
		return
	...
	if not _require(DirAccess.make_dir_recursive_absolute(output_absolute.path_join("images")) == OK, "Could not create 1444 evidence directory."):
		_finish(fixture)
		return
	paused = false
...
func _load_capture_fixture(wall_record: Dictionary, roof_record: Dictionary) -> Dictionary:
	...
	if not bool(result.get("ok", false)):
		baseline.free()
		return result
	var prototype := result.node as D21444CroakerStandaloneHeroPrototype
	prototype.name = "CaptureOnly_D2_1444_Croaker_StandaloneHero"
	prototype.set_meta("capture_only_replacement", true)
	var fixture := Node3D.new()
	fixture.name = "D2_1444_IsolatedNeutralGroundCaptureFixture"
	fixture.set_meta("capture_only", true)
	fixture.set_meta("live_world_loaded", false)
	fixture.set_meta("neutral_ground_top_y_m", 2.35)
	...
	return {
		...
		"binding": {
			...
			"fixture_contract": {
				...
				"neutral_ground_top_y_m": 2.35,
				"neutral_ground_extent_m": [130.0, 130.0],
				"exact_source_wall_triangles": 44,
				"exact_flat_source_roof_triangles": 10,
				"baseline_collision_triangles": 54,
...
func _build_neutral_ground() -> Node3D:
	...
	record.position = Vector3(-363.0, 2.20, -48.0)
...
func _capture_stills(fixture: Node3D, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, binding: Dictionary, wall_record: Dictionary) -> void:
	var prototype := binding.prototype as D21444CroakerStandaloneHeroPrototype
	...
	var manifest := {
		"schema_version": "ti.d2-1444-croaker-standalone-hero-evidence/1",
		...
		"target": {
			"canonical_name": "1444 Croaker Court",
			...
			"source_base_wall_top_y_m": [2.521, 8.521],
		},
		"truth_boundary": {
		...
		"capture_command": ".tools/godot/4.7.2/Godot.app/Contents/MacOS/Godot --path . --display-driver macos --rendering-method forward_plus --rendering-driver metal --audio-driver Dummy --resolution 1440x900 --script game/tests/d2_1444_croaker_standalone_hero_capture.gd -- --capture-mode=stills",
		"generator": "game/tests/d2_1444_croaker_standalone_hero_capture.gd",
		"generator_sha256": FileAccess.get_sha256("res://game/tests/d2_1444_croaker_standalone_hero_capture.gd"),
		...
		"lighting": {"default_sun_rotation_degrees": _vector3(DEFAULT_SUN), "changed_sun_rotation_degrees": _vector3(CHANGED_SUN), "changed_light_pair_id": "03-wsw-oblique-changed-light"},
		...
		"reviewer_instruction": "Open all six PNGs at original detail before reading labels. Against 1431 Halibut w95934135, 1436 Chinook w95934137, and 1449 Croaker w95934120, decide whether the after views are top-1 1444 using at least two supported cues: blank upper end over undercroft plus the sparse return/far-end balcony hierarchy. Check the shallow dark roof/eaves and reused stucco micro-surface in default and changed light without relying on ochre hue, address, location, or source ID. Treat all opening, undercroft, pier, balcony, rail, eave, and roof-rise dimensions/counts as reversible inference.",
	}
	if not _write_json(OUTPUT.path_join("capture-manifest.json"), manifest):
		_fail("Could not write 1444 capture manifest.")
		return
	print("PASS: captured %d native grounded 1444 same-pose baseline/prototype stills; recognition and believability remain pending independent review" % captures.size())
...
func _capture_motion(fixture: Node3D, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, binding: Dictionary, wall_record: Dictionary) -> void:
	...
	var posed := await _settle_and_aim(player, hud, requested, TARGET_POINT, "wsw-stock-walk-start")
	if not _require(bool(posed.get("ok", false)), str(posed.get("message", "Motion pose failed."))):
		return
	var los := _center_los(player, TARGET_POINT, wall_record, "prototype:%s" % WALL_KEY, 8, "wsw-stock-walk-start")
	...
	if not _require(bool(review_sheet.get("ok", false)), str(review_sheet.get("message", "Motion review contact sheet failed."))):
		return
	var manifest := {
		"schema_version": "ti.d2-1444-croaker-standalone-motion-evidence/1",
		...
		"capture_command": ".tools/godot/4.7.2/Godot.app/Contents/MacOS/Godot --path . --display-driver macos --rendering-method forward_plus --rendering-driver metal --audio-driver Dummy --resolution 1280x800 --write-movie evidence/first-playable/d2-1444-croaker-standalone-hero-2026-09-04/%s --fixed-fps 30 --script game/tests/d2_1444_croaker_standalone_hero_capture.gd -- --capture-mode=motion" % MOTION_FILE,
		"generator": "game/tests/d2_1444_croaker_standalone_hero_capture.gd",
		"generator_sha256": FileAccess.get_sha256("res://game/tests/d2_1444_croaker_standalone_hero_capture.gd"),
		...
		"architecture_under_review": "entity-specific blank-end undercroft on runs 8/9, sparse returns on runs 11/16, and far-end balcony stack on run 17",
		"samples": samples,
		"visual_verdict": "pending_independent_bar_raiser_review",
	}
	if not _write_json(OUTPUT.path_join("motion-capture.json"), manifest):
		_fail("Could not write 1444 motion manifest.")
		return
	print("PASS: drove a %.3f m, %.1f s grounded stock-player walk beside standalone 1444 with zero transform writes and recoveries; movie finalization remains post-close" % [displacement, float(MOTION_PHYSICS_FRAMES) / MOTION_PHYSICS_HZ])
...
func _write_motion_review_sheet(images: Array[Image]) -> Dictionary:
	...
	var relative := "images/d2-1444-croaker-wsw-motion-review-sheet.png"
	var path := OUTPUT.path_join(relative)
	if sheet.save_png(path) != OK:
		return {"ok": false, "message": "Could not save the 1444 motion review contact sheet."}
	return {
		"ok": true,
		"file": relative,
...
func _finalize_motion_manifest() -> void:
	...
	if not _write_json(manifest_path, manifest):
		_fail("Could not finalize 1444 motion manifest.")
		return
	print("PASS: finalized 1444 motion movie sha256=%s bytes=%d" % [expected_hash, expected_bytes])
...
func _set_baseline_enabled(nodes: Array, enabled: bool) -> void:
	for node_value: Variant in nodes:
		...
		for body_value: Variant in node.find_children("*", "CollisionObject3D", true, false):
			...
			if not body.has_meta("d2_1444_capture_original_collision_layer"):
				body.set_meta("d2_1444_capture_original_collision_layer", body.collision_layer)
			body.collision_layer = int(body.get_meta("d2_1444_capture_original_collision_layer", 0)) if enabled else 0
...
func _save_still(fixture: Node3D, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, view: Dictionary, stage: String, extra: Dictionary) -> Dictionary:
	...
	var metadata := {"id": capture_id, "stage": stage, "target": "1444-croaker-court", "source_identity": SOURCE_KEY, "role": str(view.role), "recognition_verdict": "pending_independent_bar_raiser_review", "believability_verdict": "pending_independent_bar_raiser_review", "confusion_set_source_ids_frozen_before_capture": CONFUSION_SET, "reference_supported_cues_frozen_before_capture": FROZEN_SUPPORTED_CUES, "file": relative, "sha256": FileAccess.get_sha256(path), "bytes": FileAccess.get_file_as_bytes(path).size(), "dimensions": [image.get_width(), image.get_height()], "sample_unique_colors": int(sample.unique_colors), "sample_luminance_range": float(sample.luminance_range), "hud": hud_state, "player_visible": player.visible, "debug_labels_visible": false, "source_photography_in_frame": false, "lighting_changed": not (view.sun as Vector3).is_equal_approx(DEFAULT_SUN), "sun_rotation_degrees": _vector3(sun.rotation_degrees), "full_entity_required_and_in_frame": true, "protected_untreated_faces_intentionally_not_hidden": true, "capture_only_prototype_swap": stage == "after"}
	metadata.merge(extra, true)
	print("D2_1444_CAPTURE: id=%s grounded=%s frame=%.3fx%.3f sha256=%s" % [capture_id, metadata.physics_grounded, metadata.target_projected_width_ratio, metadata.target_projected_height_ratio, metadata.sha256])
	return {"ok": true, "metadata": metadata}


func _package_hashes() -> Dictionary:
	var paths := [CONFIG_PATH, CHUNK_PATH, PACKET_PATH, "res://game/scripts/world/facades/site_12_housing_kit.gd", "res://game/scripts/world/facades/d2_1444_croaker_standalone_hero_prototype.gd", PROTOTYPE_SCENE_PATH, "res://game/resources/materials/world/site_12_housing/site_12_ochre_tan_stucco.tres", "res://game/resources/materials/world/site_12_housing/site_12_protected_neutral.tres", "res://game/resources/materials/world/site_12_housing/site_12_dark_roof_surrogate.tres", "res://game/resources/materials/world/site_12_housing/site_12_void_shadow.tres", "res://game/resources/materials/world/site_12_housing/site_12_opaque_opening.tres", "res://game/resources/materials/world/site_12_housing/site_12_dark_metal.tres", "res://game/resources/textures/world/site_12_housing/warm_ivory_mineral_albedo_v2.png", "res://game/resources/textures/world/polyhaven/bitumen/bitumen_diff_1k.jpg"]
	var hashes := {}
...
func _prototype_topology(prototype: Node3D) -> Dictionary:
	return {"mesh_instances": int(prototype.get_meta("mesh_instances", 0)), "surfaces": int(prototype.get_meta("surfaces", 0)), "visual_triangles": int(prototype.get_meta("visual_triangles", 0)), "visual_batch_triangles": prototype.get_meta("visual_batch_triangles", {}), "static_bodies": int(prototype.get_meta("static_bodies", 0)), "shapes": int(prototype.get_meta("shapes", 0)), "collision_triangles": int(prototype.get_meta("collision_triangles", 0)), "deterministic_signature": str(prototype.get_meta("deterministic_signature", "")), "undercroft_void_count": int(prototype.get_meta("undercroft_void_count", 0)), "undercroft_pier_count": int(prototype.get_meta("undercroft_pier_count", 0)), "sparse_opening_count": int(prototype.get_meta("sparse_opening_count", 0)), "balcony_void_count": int(prototype.get_meta("balcony_void_count", 0)), "rail_picket_count": int(prototype.get_meta("rail_picket_count", 0)), "mapped_eave_count": int(prototype.get_meta("mapped_eave_count", 0))}
...
func _on_timeout() -> void:
	if not _finished:
		_fail("1444 Croaker native evidence timed out after %.0f seconds." % TIMEOUT_SECONDS)
		_finish(null)
