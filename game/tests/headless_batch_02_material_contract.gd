...
func _run() -> void:
	...
	or not _require(_active_building_1_and_oasis_contract(builder), "Building 1 exact active hero or Oasis generic control drifted."):
		_finish()
		return
	print("PASS: six evidence-bounded Batch 02 material prototypes remain byte-stable and motif-free; independently accepted fields are attached only on Navy Chapel runs 9..10 and YMCA aggregate/background runs 3..11, while all other Batch 02 receivers, collision, navigation, spray, generated bytes, accepted prior facades, exact-current Building 1 hero, generic Oasis, and non-targets remain unchanged")
	_finish()
