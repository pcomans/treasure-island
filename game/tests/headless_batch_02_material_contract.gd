...
const EXPECTED_BUILDING_3_SIGNATURE := "737a674d63824d574e28c76f75c86212dfe126c3204870cdb2d0452d5bcf6337"
...
func _run() -> void:
	...
	for index in TARGETS.size():
		...
		first.free()
		second.free()
	if not _require(_prior_accepted_signatures_match(builder), "An accepted Hawkins, Building 3, or Isle House signature changed.") \
	or not _require(_blocked_targets_remain_unattached(builder), "Museum or Oasis gained runtime material/facade content."):
		_finish()
		return
	print("PASS: six evidence-bounded Batch 02 material prototypes remain byte-stable and motif-free; independently accepted fields are attached only on Navy Chapel runs 9..10 and YMCA aggregate/background runs 3..11, while all other Batch 02 receivers, collision, navigation, spray, generated bytes, accepted prior facades, Museum, Oasis, and non-targets remain unchanged")
	_finish()
...
func _blocked_targets_remain_unattached(builder: WorldChunkBuilder) -> bool:
	var controls := [
		{"chunk": "res://generated/world/chunks/x_-1__z_2.json", "key": "building:r16681702:wall"},
		{"chunk": "res://generated/world/chunks/x_0__z_1.json", "key": "building:w291189918:wall"},
	]
	for value: Variant in controls:
		var control := value as Dictionary
		var chunk := JSON.parse_string(FileAccess.get_file_as_string(str(control.chunk))) as Dictionary
		...
		if not bool(result.get("ok", false)):
			...
			return false
		var node := result.node as Node3D
		var matches := node.get_child_count() == 2 \
			and (node.find_children("*Facade*", "Node", true, false) as Array).is_empty() \
			and (node.get_node("Mesh") as MeshInstance3D).mesh.surface_get_material(0).resource_name == "building_wall"
		node.free()
		if not matches:
			return false
	return true
