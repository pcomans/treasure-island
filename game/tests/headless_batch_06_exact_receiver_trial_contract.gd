...
func _run() -> void:
	...
	if not _require(bool(whole.get("ok", false)), str(whole.get("message", "Whole-island accepted-material load failed."))):
		_finish()
		return
	print("PASS: Batch 06 scopes remain independently accepted; historical Building 1 material/module evidence remains sealed while accepted Building 1/Building 3/Isle House/Navy Chapel/B201 runtime replacements remain exact, protected complements stay unchanged, and world topology is 735/950/964/66,636/466")
	_finish()
