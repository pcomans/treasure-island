...
const EXPECTED_REGISTRY_SHA256 := "dce268c1547e4e4620faff9d59110ee1214a9a2121c1f83b3eb1c865339360ab"
const EXPECTED_UNITS := 213
const EXPECTED_RECEIVERS := 214
const EXPECTED_SOURCE_RECORDS := 215
const EXPECTED_RUNS := 4971
const EXPECTED_LEGACY_ADAPTERS := 10
const EXPECTED_ACTIVE_ADAPTERS := 4
...
func _run() -> void:
	...
	or not _require(str(registry.get("schema_version", "")) == "ti.facade-runtime-registry/4", "Runtime facade registry schema drifted.") \
	...
	if not _failed:
		print("PASS: facade recognition registry is package-safe and fail-closed: 213 physical units / 214 direct wall receivers / 215 source records / 4,971 wall runs / 5/213 independently accepted reference-recognizable units / 10 claim-neutral legacy adapters + 4 exact-current active adapters / 2 separated identity corrections; SHA-256 %s" % EXPECTED_REGISTRY_SHA256)
	_finish()
...
func _validate_counts(registry: Dictionary) -> void:
	...
	_require(int(counts.get("legacy_adapter_receivers", -1)) == EXPECTED_LEGACY_ADAPTERS, "Legacy-adapter count is not 10.")
	_require(int(counts.get("active_runtime_adapter_receivers", -1)) == EXPECTED_ACTIVE_ADAPTERS, "Active runtime-adapter count is not 4.")
	...
	_require(int(reference_claims.get("accepted", -1)) == 5 and int(reference_claims.get("not_evaluated", -1)) == EXPECTED_UNITS - 5, "Reference-recognition aggregate is not exactly 5/213.")
	...
	accepted_ids.sort()
	expected_ids.sort()
	_require(int(metric.get("numerator", -1)) == 5 and int(metric.get("denominator", -1)) == EXPECTED_UNITS and str(metric.get("display", "")) == "5/213", "Physical-unit recognition metric is not exactly 5/213.")
...
func _validate_units(registry: Dictionary) -> void:
	...
	_require(building_1_main_seen and building_1_tower_seen and building_3_seen, "Building 1 main/tower or Building 3 is not separately represented.")
...
func _validate_runtime_adapters(registry: Dictionary) -> void:
	...
	var receiver_keys := {}
	_require(legacy_adapters.size() == EXPECTED_LEGACY_ADAPTERS, "Runtime legacy adapter array is not 10 entries.")
	_require(active_adapters.size() == EXPECTED_ACTIVE_ADAPTERS, "Runtime active adapter array is not four entries.")
	...
	for adapter_value: Variant in active_adapters:
		var adapter := adapter_value as Dictionary
		var receiver_key := str(adapter.get("receiver_key", ""))
		_require(receiver_key in ["building-composite:w1249412094:w1282547787:wall", "building:r16681702:wall", "building:w1222720021:wall", "building:w34313540:wall"], "Active adapter targets an unexpected receiver: %s." % receiver_key)
		...
		if receiver_key == "building:w34313540:wall":
			...
			_require(int(geometry.get("world_records", -1)) == 735 and int(geometry.get("world_mesh_instances", -1)) == 940 and int(geometry.get("world_surfaces", -1)) == 954 and int(geometry.get("world_triangles", -1)) == 64118 and int(geometry.get("world_static_bodies", -1)) == 466 and int(geometry.get("world_shapes", -1)) == 466, "Isle House accepted world topology drifted.")
