...
const EXPECTED_ACTIVE_ADAPTERS := 25
const EXPECTED_RUNTIME_ADAPTERS := 34
...
func _run() -> void:
	...
	or not _require(str(registry.get("schema_version", "")) == "ti.facade-runtime-registry/26", "Runtime facade registry schema drifted.") \
	...
	if not _failed:
		print("PASS: facade recognition registry is package-safe and fail-closed: 213 physical units / 214 direct wall receivers / 215 source records / 4,971 wall runs / 26/213 independently accepted reference-recognizable units / 9 claim-neutral legacy adapters + 25 exact-current active adapters / 2 separated identity corrections; SHA-256 %s" % EXPECTED_REGISTRY_SHA256)
	_finish()
...
func _validate_counts(registry: Dictionary) -> void:
	...
	_require(int(reference_claims.get("accepted", -1)) == 26 and int(reference_claims.get("not_evaluated", -1)) == EXPECTED_UNITS - 26, "Reference-recognition aggregate is not exactly 26/213.")
	...
	accepted_ids.sort()
	expected_ids.sort()
	_require(int(metric.get("numerator", -1)) == 26 and int(metric.get("denominator", -1)) == EXPECTED_UNITS and str(metric.get("display", "")) == "26/213", "Physical-unit recognition metric is not exactly 26/213.")
...
func _validate_runtime_adapters(registry: Dictionary) -> void:
	...
	_require(current_topology_adapter_ids == ["active-adapter:mariner-1202-live:building:w96215651:wall"], "1202 is not the sole current-integration topology authority.")
