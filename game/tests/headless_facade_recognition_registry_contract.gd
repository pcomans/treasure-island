...
const EXPECTED_ACTIVE_ADAPTERS := 22
const EXPECTED_RUNTIME_ADAPTERS := 31
...
func _run() -> void:
	...
	or not _require(str(registry.get("schema_version", "")) == "ti.facade-runtime-registry/23", "Runtime facade registry schema drifted.") \
	...
	if not _failed:
		print("PASS: facade recognition registry is package-safe and fail-closed: 213 physical units / 214 direct wall receivers / 215 source records / 4,971 wall runs / 23/213 independently accepted reference-recognizable units / 9 claim-neutral legacy adapters + 22 exact-current active adapters / 2 separated identity corrections; SHA-256 %s" % EXPECTED_REGISTRY_SHA256)
	_finish()
...
func _validate_counts(registry: Dictionary) -> void:
	...
	_require(int(reference_claims.get("accepted", -1)) == 23 and int(reference_claims.get("not_evaluated", -1)) == EXPECTED_UNITS - 23, "Reference-recognition aggregate is not exactly 23/213.")
	...
	accepted_ids.sort()
	expected_ids.sort()
	_require(int(metric.get("numerator", -1)) == 23 and int(metric.get("denominator", -1)) == EXPECTED_UNITS and str(metric.get("display", "")) == "23/213", "Physical-unit recognition metric is not exactly 23/213.")
...
func _validate_runtime_adapters(registry: Dictionary) -> void:
	...
	_require(current_topology_adapter_ids == ["active-adapter:northpoint-1239-live:building:w96215658:wall"], "1239 is not the sole current-integration topology authority.")
...
func _runtime_asset_closure_is_clean(path: String, visited: Dictionary) -> bool:
	...
	if path in ["res://game/scripts/world/facades/northern_canopy_free_live_replacement.gd", "res://game/scripts/world/facades/mariner_1206_live_replacement.gd", "res://game/scripts/world/facades/mariner_1219_live_replacement.gd", "res://game/scripts/world/facades/mariner_1212_live_replacement.gd", "res://game/scripts/world/facades/bayside_1220_live_replacement.gd", "res://game/scripts/world/facades/northpoint_1239_live_replacement.gd", "res://game/scripts/world/facades/northpoint_1238_live_replacement.gd"]:
		var expected_hash: String = "e397a0879a8beb8eca7e8ec32e80c40652cfb1b5b67a50bf6f52952f4ae44aa5" if path.ends_with("/northpoint_1239_live_replacement.gd") else "1edb75f24c3de6b4c6c780c9bb6b478225925453872e11c76f82fece8a146c38" if path.ends_with("/bayside_1220_live_replacement.gd") else "c0941706e3cee909b260e278a07d846932b805aaeb5e12440463e43678b3513c" if path.ends_with("/mariner_1212_live_replacement.gd") else "c6a0c16cc762d33c62c3f3885d865999be5e65e148f56a66e9da348e6cbfb58a" if path.ends_with("/mariner_1219_live_replacement.gd") else "88e0858a2b9d3559c8d2e766874f93cd44335f89c232e8115ef7c7f97ecc951b" if path.ends_with("/mariner_1206_live_replacement.gd") else "8967229a37be5401962132b846b08f1f527fb27f6aae3cd2d6d97eb70f608b82" if path.ends_with("/northern_canopy_free_live_replacement.gd") else "fef8305341da71b88c1fa90a5ccdd9bba98afc10f999de7208cc6373ff7ba9c3"
