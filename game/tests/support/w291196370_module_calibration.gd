...
const TRIM := preload("res://game/resources/materials/world/w291196370/w291196370_trim.tres")
const GLASS := preload("res://game/resources/materials/world/w291196370/w291196370_glass.tres")
const ENTRY := preload("res://game/resources/materials/world/w291196370/w291196370_entry.tres")
const SERVICE := preload("res://game/resources/materials/world/w291196370/w291196370_service.tres")
...
static func build(record: Dictionary, registry: Dictionary) -> Dictionary:
	if not _record_matches(record, registry):
		return {"ok": false, "message": "Exact w291196370 receiver record does not match the detached calibration contract."}
	var contracts := {}
	...
	var root := Node3D.new()
	root.name = "DetachedW291196370ModuleCalibration"
	root.set_meta("controlled_calibration", true)
	root.set_meta("runtime_attachment", false)
	root.set_meta("source_key", "w291196370")
	root.set_meta("receiver_key", "building:w291196370:wall")
	...
	for placement_value: Variant in registry.get("placements", []):
		...
		var module := _build_module(motif_id, contract)
		if module == null:
			root.free()
			return {"ok": false, "message": "Unsupported motif %s." % motif_id}
		...
		if motif_id == "W291196370-WINSTACK":
			origin.y = float(frame.base_y) + float(contract.get("center_height_above_base_assumption_m", 3.15))
		else:
			origin.y = float(frame.base_y)
		module.transform = Transform3D(Basis(tangent, Vector3.UP, normal), origin)
		for key: String in ["placement_id", "motif_id", "face", "region", "host_material_id", "mapping_id", "evidence_status", "coordinate_status", "count_status", "cadence_status"]:
			module.set_meta(key, placement.get(key, ""))
...
static func _build_module(motif_id: String, contract: Dictionary) -> Node3D:
	match motif_id:
		"W291196370-WINSTACK":
			return _build_winstack(contract)
		"W291196370-ENTRY":
			return _build_entry(contract)
		"W291196370-SERVICE":
			return _build_service(contract)
	return null


static func _build_winstack(contract: Dictionary) -> Node3D:
	...
	module.add_child(_box("CompleteWindowStackGlass", Vector3(0.0, 0.0, 0.025), Vector3(opening.x, opening.y, 0.05), GLASS))
	_add_frame_bars(module, "CompleteWindowStackFrame", Vector3(0.0, 0.0, 0.06), opening, TRIM, thickness)
	for row in range(1, int(contract.pane_rows)):
		var y := -opening.y * 0.5 + opening.y * float(row) / float(contract.pane_rows)
		module.add_child(_box("CompleteWindowStackMullion%02d" % row, Vector3(0.0, y, 0.065), Vector3(opening.x, 0.085, 0.07), TRIM))
	return module


static func _build_entry(contract: Dictionary) -> Node3D:
	var module := Node3D.new()
	var opening_values := contract.opening_m as Array
	var opening := Vector2(float(opening_values[0]), float(opening_values[1]))
	var center_y := float(contract.base_clearance_assumption_m) + opening.y * 0.5
	module.add_child(_box("CompleteEntryLeaf", Vector3(0.0, center_y, 0.025), Vector3(opening.x, opening.y, 0.05), ENTRY))
	_add_frame_bars(module, "CompleteEntryFrame", Vector3(0.0, center_y, 0.06), opening, TRIM, float(contract.frame_thickness_m))
	return module


static func _build_service(contract: Dictionary) -> Node3D:
	var module := Node3D.new()
	var opening_values := contract.opening_m as Array
	var opening := Vector2(float(opening_values[0]), float(opening_values[1]))
	var frame_values := contract.frame_opening_m as Array
	var frame_opening := Vector2(float(frame_values[0]), float(frame_values[1]))
	var center_y := float(contract.base_clearance_assumption_m) + frame_opening.y * 0.5
	module.add_child(_box("CompleteServiceLeaf", Vector3(0.0, center_y, 0.025), Vector3(opening.x, opening.y, 0.05), SERVICE))
	_add_frame_bars(module, "CompleteServiceFrame", Vector3(0.0, center_y, 0.06), frame_opening, TRIM, float(contract.frame_thickness_m))
	for seam in int(contract.leaf_seam_count):
		var y := center_y - opening.y * 0.5 + 0.255 + seam * float(contract.leaf_seam_spacing_m)
		module.add_child(_box("CompleteServiceLeafSeam%02d" % seam, Vector3(0.0, y, 0.075), Vector3(opening.x - 0.15, 0.035, 0.035), TRIM))
	return module
