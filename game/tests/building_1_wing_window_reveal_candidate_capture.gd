...
## reveal candidate. Both stages use ordinary main, the actual stock player rig,
...
func _load_main_exact(stage: String) -> Dictionary:
	...
	var expected_triangles := 69252 if stage == "baseline" else 69612
...
func _validate_bindings(world: WorldLoader, stage: String) -> Dictionary:
	var expected := {
		WALL_KEY: {"meshes": 6, "surfaces": 6, "triangles": 8806 if stage == "baseline" else 9166, "faces": 2046},
	...
	if reveal_triangles != (168 if stage == "baseline" else 528):
...
func _b1_capture_stills(main: GameMain, world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, stage: String, stage_root: String, bindings: Dictionary) -> void:
	...
	manifest.merge({
		"schema_version": "ti.building-1-wing-window-reveal-candidate-stills/1",
	...
	or not _require(_write_new_json(manifest_path, manifest), "Could not write %s still manifest." % stage):
		return
	print("PASS: captured %s B1 reveal stage with 22 native Metal PNGs: matched default/changed/grayscale wing views, protected return, tower, real spray, supported roof landing, whole context, and diagnostic overview" % stage)
...
func _b1_capture_motion(main: GameMain, world: WorldLoader, player: PlayerController, hud: GameHUD, sun: DirectionalLight3D, stage: String, stage_root: String, side: String, bindings: Dictionary) -> void:
	...
	var manifest := _common_manifest(stage, bindings, runtime)
	manifest.merge({
		"schema_version": "ti.building-1-wing-window-reveal-candidate-motion/1",
		"side": side,
...
func _common_manifest(stage: String, bindings: Dictionary, runtime: Variant) -> Dictionary:
	return {
		...
		"generator": "game/tests/building_1_wing_window_reveal_candidate_capture.gd",
