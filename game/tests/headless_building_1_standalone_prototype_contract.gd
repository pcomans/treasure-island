...
const INTERRUPTED_R133351_HASHES := {
...
func _run() -> void:
	...
	or not _require(_hashes_match(INTERRUPTED_R133351_HASHES), "Interrupted r133351 work changed during the Building 1 round."):
		_finish()
		return
...
func _runtime_isolation_matches() -> bool:
	...
	var recognizable := FileAccess.get_file_as_string("res://game/scripts/world/facades/building_1_recognizable_facade.gd")
	...
	for module_token in ["B1-WING-W", "B1-CENTRAL-W", "B1-PAV-W", "B1-BASE-O", "B1-DOOR-BLUE", "B1-BAND"]:
		if module_token in live_field:
			return false
	var registry_value: Variant = JSON.parse_string(FileAccess.get_file_as_string("res://game/resources/facades/building_1_recognizability_placements.json"))
	var registry := registry_value as Dictionary if registry_value is Dictionary else {}
	var truth := registry.get("truth_boundary", {}) as Dictionary
	return FileAccess.file_exists("res://game/resources/facades/building_1_exact_receiver_calibration.json") \
		and FileAccess.file_exists("res://game/scripts/world/facades/building_1_live_field.gd") \
		and FileAccess.file_exists("res://game/resources/facades/building_1_recognizability_placements.json") \
		and "BUILDING_1_RECOGNIZABLE_FACADE" in builder \
		and "building_1_standalone_prototype_set.tscn" in recognizable \
