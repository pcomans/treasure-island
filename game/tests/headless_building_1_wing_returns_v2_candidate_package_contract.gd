...
## returns v2 candidate. This intentionally checks instantiated ordinary-world
...
## companion Python package verifier binds native signed-release world telemetry.
...
const EXPECTED_PCK_SHA256 := "59b37cf856dd1ece6aaadeee5c6278327997394339bb8450253193cbd89728da"
...
func _run() -> void:
	...
	_require(GameMain.EXPECTED_TRIANGLES == 69252 and GameMain.EXPECTED_MESHES == 959 and GameMain.EXPECTED_SURFACES == 974, "Mounted main must retain the accepted historical smoke oracle; candidate telemetry is separately scoped.")
	...
	_validate_records()
	if not _failed:
		print("PASS: mounted Building 1 wing-return v2 candidate PCK is exact at v9 9/213; remap-loaded PCK builders produce B1 13/13/12151 with 168 accepted center reveal triangles and 8244 trim triangles while D2 ownership/collision remains intact, native full-world telemetry is checked separately by the companion package verifier")
