...
## B1 v2 candidate world: accepted 69252 + measured 1440 render-only triangles.
## All other checks are identical to the accepted full-runtime oracle.
...
func _run() -> void:
	...
	or not _require(evidence.mesh_instances == EXPECTED_RUNTIME_MESHES and evidence.surfaces == EXPECTED_RUNTIME_SURFACES and evidence.triangles == EXPECTED_RUNTIME_TRIANGLES, "RuntimeEvidence must include accepted-through-1441 content: 959 meshes / 974 surfaces / 70,692 triangles with bounded B1 returns v2.") \
...
func _batch_06_accepted_fields_match_contract(records: Array, runtime_counts: Dictionary) -> bool:
	...
	return seen.size() == ACCEPTED_MATERIAL_RUN_TRIALS.BATCH_06_ACCEPTED_TARGETS.size() \
		...
		and int(runtime_counts.surfaces) - int(runtime_counts.meshes) \
			== ACCEPTED_MATERIAL_RUN_TRIALS.TARGETS.size() - 1 + batch_06_surface_delta + EXPECTED_B201_HOST_PARTITION_SURFACE_DELTA + EXPECTED_B225_HOST_PARTITION_SURFACE_DELTA
