...
func _run() -> void:
	...
	or not _require(_museum_and_oasis_remain_unattached(builder), "Museum or Oasis gained runtime facade content."):
		_finish()
		return
	print("PASS: 17 evidence-bounded Batch 04 prototype fields remain byte-stable, alias-filtered, facade-scale-motif-free, and temporally explicit; only Dormitory 369 warm wall is attached on exact runs 0..5, while Education 29B, every other Batch 04 receiver, collision, navigation, spray, generated bytes, Museum, Oasis, and non-targets remain unchanged")
	_finish()
...
func _museum_and_oasis_remain_unattached(builder: WorldChunkBuilder) -> bool:
	var controls := [
