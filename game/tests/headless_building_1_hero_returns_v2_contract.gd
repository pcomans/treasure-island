...
## Current v2 candidate expectations; the frozen v1 contract is unchanged.
...
func _run() -> void:
	...
	for key in KEYS:
		...
		first_node.free()
		second_node.free()
	_require(combined_triangles == 12151, "Building 1 main+roof+tower combined triangles are not exactly 12,151: %d" % combined_triangles)
	print("BUILDING_1_HERO_COMPONENT_TOPOLOGY=%s" % JSON.stringify(measured))
	await _whole_world_contract()
	if not _failed:
		print("PASS: Building 1 wing-returns v2 candidate preserves four source identities, exact non-trim/tower/roof geometry, collision, and horizontal plans at 13 meshes / 13 surfaces / 12,151 triangles")
...
func _whole_world_contract() -> void:
	...
	if evidence != null:
		...
		_require(evidence.playable_rows == 735 and evidence.context_rows == 4 \
			and evidence.mesh_instances == 959 and evidence.surfaces == 974 and evidence.triangles == 70692 \
