...
## Fail-closed verifier for the disposable Building 1 wing-window reveal A/B
...
## fixed-60 controller telemetry, and the bounded +360-triangle visual delta.

const PACKET_ID := "building-1-wing-window-reveal-candidate-2026-09-08-001"
const CAPTURE_DATE := "2026-09-08"
const CAPTURE_SIZE := Vector2i(1440, 900)
const RECEIPT_NAME := "rehearsal-verification-receipt.json"
const CAPTURE_GENERATOR := "res://game/tests/building_1_wing_window_reveal_candidate_capture.gd"
const CAPTURE_GENERATOR_SHA256 := "20363d664e268e4760ba8d5817c71553a9237a6df442931185fdc69c752f05d4"
const PACKAGE_CONTRACT := "res://game/tests/headless_building_1_wing_window_reveal_candidate_package_contract.gd"
const PACKAGE_CONTRACT_SHA256 := "fdf42f697312c0149ebbfbb385ece6cc2fa675f623769c4634f37e3311450190"
const SOURCE_AUDIT := "res://evidence/reviews/building-1-wing-window-reveal-candidate-2026-09-08-INDEPENDENT_SOURCE_AUDIT.md"
const SOURCE_AUDIT_SHA256 := "c70a364dc90746f5e5b6f2e1021f68c8cd94dcc52761516538d19a3edaed24a0"
const SOURCE_AUDIT_TOKEN := "PASS_FOR_B1_REVEAL_EVIDENCE_CAPTURE"
...
const CANDIDATE_PCK_SHA256 := "835cfc74b5e4662abdab27d023f0dd0a00feaf06dcc19a73657bc6631d47474b"
const BASELINE_CONFIG_SHA256 := "7b53847c627d6f0a0d4ebefcc790e8fd3bcaeee6fbdebbf5c6a85f2aeb4a5806"
const CANDIDATE_CONFIG_SHA256 := "fca8feba243c4b1b1747050852ab2db7102aae668838ad8265524caa20cc6fc1"
const BASELINE_ADAPTER_SHA256 := "e5b11fc7f971ea33bb38c84d3106f8883de0486774f58a5a8310d49d56aa55a5"
const CANDIDATE_ADAPTER_SHA256 := "1b3e9086b9a5628a68fe684736010277219b02f931d09e03f1442be15afe35a8"
const CANDIDATE_EXE_SHA256 := "a2a1b0a6a20be0c6fec9f4cef659540c324ae645a5c3c69683b353c061afa945"
...
const CANDIDATE_BUILD_ROOT := "res://build/building-1-wing-window-reveal-candidate-2026-09-08-001/Treasure Island First Playable.app"
...
const EXPECTED_TOPOLOGY := {
	...
	"candidate": {"rows": 735, "meshes": 959, "surfaces": 974, "triangles": 69612, "bodies": 466, "shapes": 466},
}
...
const EXPECTED_MAIN_WALL_TRIANGLES := {"baseline": 8806, "candidate": 9166}
const EXPECTED_REVEAL_TRIANGLES := {"baseline": 168, "candidate": 528}
...
func _run() -> void:
	...
	if mode == "create":
		...
		or expected_receipt_sha != receipt_sha or FileAccess.get_file_as_string(receipt_path) != receipt_text:
			...
			_finish()
			return
	print("PASS: rehearsal-only B1 reveal A/B closure is exact: 92 original PNGs + 6 manifests, byte-rederived grayscale, zero paired pose/controller-tick drift, intended +360 visual triangles only; receipt=%s" % receipt_sha)
	_finish()
...
func _validate(root_path: String, mode: String) -> Dictionary:
	...
	var receipt := {
		"schema_version": "ti.building-1-wing-window-reveal-candidate-rehearsal-verification/1",
		"packet_id": PACKET_ID,
		...
		"capture_generator": "game/tests/building_1_wing_window_reveal_candidate_capture.gd",
		"capture_generator_sha256": CAPTURE_GENERATOR_SHA256,
		"evidence_contract": "game/tests/headless_building_1_wing_window_reveal_candidate_evidence_contract.gd",
		"evidence_contract_sha256": FileAccess.get_sha256("res://game/tests/headless_building_1_wing_window_reveal_candidate_evidence_contract.gd"),
		"mounted_candidate_pck_contract": "game/tests/headless_building_1_wing_window_reveal_candidate_package_contract.gd",
		...
		"source_audit": {"path": "evidence/reviews/building-1-wing-window-reveal-candidate-2026-09-08-INDEPENDENT_SOURCE_AUDIT.md", "sha256": SOURCE_AUDIT_SHA256, "token": SOURCE_AUDIT_TOKEN},
		...
		"bounded_delta": {"meshes": 0, "surfaces": 0, "triangles": 360, "bodies": 0, "shapes": 0, "main_wall_triangles": 360, "reveal_triangles": 360},
...
func _common_manifest_valid(manifest: Dictionary, stage: String) -> bool:
	...
	return str(manifest.get("packet_id", "")) == PACKET_ID \
		...
		and str(manifest.get("generator", "")) == "game/tests/building_1_wing_window_reveal_candidate_capture.gd" \
...
func _stills_valid(root_path: String, manifest: Dictionary, stage: String) -> bool:
	...
	or str(manifest.get("schema_version", "")) != "ti.building-1-wing-window-reveal-candidate-stills/1" \
...
func _motion_valid(root_path: String, manifest: Dictionary, stage: String, side: String) -> bool:
	if not _common_manifest_valid(manifest, stage) \
	or str(manifest.get("schema_version", "")) != "ti.building-1-wing-window-reveal-candidate-motion/1" \
