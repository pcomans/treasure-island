class_name NorthernCanopyFreeLiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1201 Bayside source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/northern_canopy_free_live_factory.gd")
const ADAPTER_ID := "active-adapter:northern-canopy-free-live:building:w96215672:wall"
const SOURCE_KEY := "w96215672"
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("northern_canopy_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("northern_canopy_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("northern_canopy_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("northern_canopy_pair", "Exact supplied 1201 wall and roof are required.", {})
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("northern_canopy_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("northern_canopy_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("northern_canopy_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("northern_canopy_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("northern_canopy_factory_shape", "Factory did not retain the exact roof result.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "NorthernCanopyFreeLiveWall" if wall else "NorthernCanopyFreeLiveRoof"
