extends Node3D
## Preserved accepted 1241 revision002 construction behind live caller callbacks.
...
const CONFIG_PATH := "res://game/resources/facades/northpoint_1241_study_002.json"
...
const TARGET_RUNS := [0,1,2,3,5,6,7,8,10,11,12]
...
func configure_records(wall: Dictionary, roof: Dictionary, source_builder: Callable, tangent_builder: Callable, baseline: bool = false) -> Dictionary:
	...
	if str(cfg.get("schema_version", "")) != "ti.northpoint-1241-study/1" or str(cfg.target.source_key) != "w96215674":
		return {"ok": false, "message": "This coherent study is 1241 only."}
	...
	if not baseline:
		...
		body.set_meta("source_keys", ["w96215674"])
...
func _add_projected_receiver(wall: Dictionary) -> void:
	...
	body.set_meta("source_run_indices", [0,1,2,3,5,6,7,8,10,11,12])
...
static func _surface_material(label: String, cfg: Dictionary) -> Material:
	if label == "CarportFascia":
		...
		fascia.resource_name = "1241_weathered_gray_canopy_fascia"
	...
	if label == ROOF_BUCKET:
		...
		roof.resource_name = "1241_shallow_gray_observed_roof"
	...
	if label in GROUND_BUCKETS:
		...
		ground.resource_name = "1241_actual_grade_" + label
	...
	if label in ["NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures"]:
		...
		material.resource_name = "1241_pale_horizontal_siding"
	...
	material.resource_name = "1241_" + label
