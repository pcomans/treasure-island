extends Node3D
## Isolated 1238 art study; deliberately not registered with the live adapter.
...
const SELF_PATH := "res://game/scripts/world/facades/northpoint_1238_study.gd"
const CONFIG_PATH := "res://game/resources/facades/northpoint_1238_study.json"
const CHUNK_PATH := "res://generated/world/chunks/x_-1__z_-4.json"
const WALL_KEY := "building:w96215669:wall"
const ROOF_KEY := "building:w96215669:roof"
const EXPECTED_CHUNK_SHA256 := "bb21618ff0bc88549e3a600d981880415817747475c2c3a0d368a7da8db4d3ac"
const TARGET_RUNS := [8,9,10,11,13,14,15,17,18,19,20,21]
const PROTECTED_RUNS := [0,1,2,3,4,5,6,7,12,16,22,23]
const PHYSICAL_BUCKETS := ["PaleCompleteTrim","BlueOpaqueGlazing","DarkOpaqueGlazing","DarkBlueClosedDoors","LocalPrivacyScreens","PaleDrainageAndFascia","DoorHardware"]
...
func configure_records(wall: Dictionary, roof: Dictionary, baseline: bool = false) -> Dictionary:
	...
	if str(cfg.get("schema_version", "")) != "ti.northpoint-1238-study/1" or str(cfg.target.source_key) != "w96215669":
		return {"ok": false, "message": "This coherent study is 1238 only."}
	...
	for label: String in ["NearHorizontalSiding", "PaleCompleteTrim", "BlueOpaqueGlazing", "DarkOpaqueGlazing", "DarkBlueClosedDoors", "LocalPrivacyScreens", "PaleDrainageAndFascia", "DoorHardware", "DrapedEntryPaths"]:
	...
	if not baseline:
		...
		body.set_meta("source_keys", ["w96215669"])
...
func _append_zone(frame: Dictionary) -> void:
	...
	for entry: Dictionary in frame.entries:
		...
		KIT.append_box(_buckets.DarkBlueClosedDoors, _point(frame, s, bottom + height / 2.0, .071), tangent, normal, width, height, .098)
		for side in [-1.0, 1.0]:
	...
	for pipe: Dictionary in frame.pipes:
		...
		for y in [float(pipe.bottom_y) + .42, float(frame.story_band_y) + .11, float(frame.top_y) - 1.0]:
	...
	KIT.append_eave_band(_buckets.PaleDrainageAndFascia, _point(frame, float(frame.length_m) / 2.0, float(frame.top_y) - .097, .015), tangent, normal, length, .145, .25)
	KIT.append_box(_buckets.PaleDrainageAndFascia, _point(frame, float(frame.length_m) / 2.0, float(frame.top_y) - .015, .21), tangent, normal, length, .038, .15)
	KIT.append_box(trim, _point(frame, float(frame.length_m) / 2.0, float(frame.story_band_y), .053), tangent, normal, length, .060, .088)
...
static func _surface_material(label: String, cfg: Dictionary) -> Material:
	...
	if label == "NearHorizontalSiding":
		...
		material.resource_name = "1238_quiet_blue_gray_horizontal_siding"
	...
	material.resource_name = "1238_" + label
