class_name Bayside1215LiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1215 Northpoint source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/bayside_1215_live_factory.gd")
const ADAPTER_ID := "active-adapter:bayside-1215-live:building:w96215666:wall"
const SOURCE_KEY := "w96215666"
const WALL_KEY := "building:w96215666:wall"
const ROOF_KEY := "building:w96215666:roof"
const TARGET_CHUNK_ID := "x_-2__z_-3"
const EXPECTED_FACTORY_SHA256 := "6613df3605004f128b22bf9f2d3a1a67c8e367c45216557c0c3ab293afb36880"
const EXPECTED_CONFIG_SHA256 := "69ec5d8fdf7a9fb93148ae459c5c570726509ea30e10a4193adb2294810b52d8"
const MAPPED_RUNS := [0,1,2,3,4,5,19]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("bayside_1215_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("bayside_1215_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("bayside_1215_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("bayside_1215_pair", "Exact supplied 1215 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("bayside_1215_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("bayside_1215_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("bayside_1215_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("bayside_1215_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("bayside_1215_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("bayside_1215_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("bayside_1215_factory_shape", "Factory did not retain both original roof resources.", wall_record)
	...
	var metadata := {
		...
		"continuity_run_indices": [0,2],
		"partial_run_ownership": {"5": {"skin_fraction_interval": [0.0,0.5], "unestablished_outer_half_protected": true}, "19": {"skin_fraction_interval": [0.15,1.0], "cropped_outer_endpoint_protected": true}},
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Bayside1215LiveWall" if wall else "Bayside1215LiveRoof"
