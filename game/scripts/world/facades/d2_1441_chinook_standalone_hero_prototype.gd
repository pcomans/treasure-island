class_name D21441ChinookStandaloneHeroPrototype
extends Node3D
...
func _ready() -> void:
	if auto_configure_from_frozen_source and not _configured:
		...
		if not bool(result.get("ok", false)):
			push_error("1441 Chinook standalone hero failed closed: %s" % str(result.get("message", "unknown error")))
...
static func build_for_records(wall_record: Dictionary, roof_record: Dictionary) -> Dictionary:
	var prototype := D21441ChinookStandaloneHeroPrototype.new()
...
func configure_from_frozen_source() -> Dictionary:
	...
	if chunk.is_empty():
		return _failure("d2_1441_source_chunk", "Frozen x_-1__z_-1 source chunk could not be loaded.")
...
func configure_records(wall_record: Dictionary, roof_record: Dictionary) -> Dictionary:
	if _configured:
		return _failure("d2_1441_duplicate_configuration", "The standalone 1441 Chinook prototype is already configured.")
	var config := _json(CONFIG_PATH)
	if not _config_contract_matches(config):
		return _failure("d2_1441_config_contract", "The standalone 1441 Chinook truth/configuration contract drifted.")
	...
	var wing_openings := SITE_12_KIT.new_bucket()
	var opening_frames := SITE_12_KIT.new_bucket()
	var recess_voids := SITE_12_KIT.new_bucket()
	...
	var wing_opening_count := 0
	var recess_void_count := 0
	var rail_picket_count := 0
	for run_index: int in TARGET_RUNS:
		var frame := SITE_12_KIT.run_frame(wall_record, run_index)
		if frame.is_empty():
			_clear_children_now()
			return _failure("d2_1441_target_run", "Mapped SSE run %d could not be resolved." % run_index)
	...
	for run_index: int in WING_RUNS:
		var frame := SITE_12_KIT.run_frame(wall_record, run_index)
		for row_y: float in [float(inference.lower_row_center_y), float(inference.upper_row_center_y)]:
			for column_index in range(int(inference.wing_columns_per_run)):
				var fraction := float(column_index + 1) / float(int(inference.wing_columns_per_run) + 1)
				var anchor := (frame.start as Vector3).lerp(frame.end as Vector3, fraction)
				anchor.y = row_y
				SITE_12_KIT.append_grouped_opening(
					...
					frame.tangent as Vector3,
					frame.normal as Vector3,
					float(inference.wing_opening_width),
					float(inference.wing_opening_height),
					float(inference.wing_opening_depth),
					float(inference.opening_frame_member),
					float(inference.opening_frame_depth),
					float(inference.opening_mullion_width)
				)
				wing_opening_count += 1

	for run_index: int in RECESS_RUNS:
		var frame := SITE_12_KIT.run_frame(wall_record, run_index)
		var bay_width := float(frame.length_m) - float(inference.void_side_margin) * 2.0
		if bay_width <= 1.0:
			_clear_children_now()
			return _failure("d2_1441_recess_width", "Mapped recessed run %d is too short for its bounded module." % run_index)
		for row_y: float in [float(inference.lower_row_center_y), float(inference.upper_row_center_y)]:
			...
			rail_picket_count += SITE_12_KIT.append_recessed_balcony_or_breezeway(
				...
				frame.normal as Vector3,
				bay_width,
				float(inference.void_height),
				float(inference.void_depth),
	...
	var specs: Array[Dictionary] = [
		...
		{"name": "MappedSSEExactStuccoWallRuns", "bucket": target_walls, "material": TARGET_STUCCO, "layers": RENDER_BUILDING_WALL},
		{"name": "ExactPlanShallowGableRoof", "bucket": roof, "material": DARK_ROOF, "layers": RENDER_WORLD_VISIBLE},
		{"name": "MappedSSEBroadEaves", "bucket": eaves, "material": DARK_ROOF, "layers": RENDER_BUILDING_WALL},
		{"name": "ProjectingWingOpaqueOpenings", "bucket": wing_openings, "material": OPAQUE_OPENING, "layers": RENDER_BUILDING_WALL},
		{"name": "ProjectingWingPaleOpeningFrames", "bucket": opening_frames, "material": TARGET_STUCCO, "layers": RENDER_BUILDING_WALL},
		{"name": "RecessedTwoLevelOpaqueVoids", "bucket": recess_voids, "material": VOID_SHADOW, "layers": RENDER_BUILDING_WALL},
		{"name": "RecessedBalconySlabs", "bucket": balcony_slabs, "material": TARGET_STUCCO, "layers": RENDER_BUILDING_WALL},
		{"name": "RecessedSimpleMetalRails", "bucket": rails, "material": DARK_METAL, "layers": RENDER_BUILDING_WALL},
	]
	...
	var metadata := {
		...
		"wing_run_indices": WING_RUNS.duplicate(),
		"recess_run_indices": RECESS_RUNS.duplicate(),
		"protected_run_indices": PROTECTED_RUNS.duplicate(),
		"protected_runs_have_facade_modules": false,
		"wing_opening_count": wing_opening_count,
		"recess_void_count": recess_void_count,
...
func _append_exact_plan_gable(bucket: Dictionary, roof_record: Dictionary, wall_record: Dictionary, inference: Dictionary) -> void:
	...
	var public_frame := SITE_12_KIT.run_frame(wall_record, 10)
