class_name Mariner1206LiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1206 Mariner source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/mariner_1206_live_factory.gd")
const ADAPTER_ID := "active-adapter:mariner-1206-live:building:w96215677:wall"
const SOURCE_KEY := "w96215677"
const WALL_KEY := "building:w96215677:wall"
const ROOF_KEY := "building:w96215677:roof"
const TARGET_CHUNK_ID := "x_-2__z_-3"
const EXPECTED_FACTORY_SHA256 := "906922118fd881a5668962c87964b8c46db32d92a5483fd072a4c2a90b521c56"
const EXPECTED_CONFIG_SHA256 := "5327998befca469042db9ac728de697e56fb311f9f889bb1bc867617425d4f97"
const MAPPED_RUNS := [11, 12, 13, 15, 16, 17, 18, 19, 21, 22, 23]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("mariner_1206_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("mariner_1206_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("mariner_1206_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("mariner_1206_pair", "Exact supplied 1206 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("mariner_1206_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("mariner_1206_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("mariner_1206_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("mariner_1206_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("mariner_1206_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("mariner_1206_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null or roof_addition == null or roof_body == null:
		wall_root.free()
		return _failure("mariner_1206_factory_shape", "Factory did not retain both original and added roof resources.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Mariner1206LiveWall" if wall else "Mariner1206LiveRoof"
