class_name Northpoint1238LiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1238 Northpoint source pair.
## It delegates authored geometry to the explicit live factory and only separates
## the existing source roof result from the wall result for normal live dispatch.

const FACTORY := preload("res://game/scripts/world/facades/northpoint_1238_live_factory.gd")
const ADAPTER_ID := "active-adapter:northpoint-1238-live:building:w96215669:wall"
const SOURCE_KEY := "w96215669"
const WALL_KEY := "building:w96215669:wall"
const ROOF_KEY := "building:w96215669:roof"
const TARGET_CHUNK_ID := "x_-1__z_-4"
const EXPECTED_FACTORY_SHA256 := "2046c934f4fdaa35f12c7e343910b26a7e1cb2d7e489adc22467bcd1b41bb45f"
const EXPECTED_CONFIG_SHA256 := "a7f733e4f67351f9919769ff1329f263efbd43ca0d39a3fb30c3ac77e626c2a9"
const MAPPED_RUNS := [8, 9, 10, 11, 13, 14, 15, 17, 18, 19, 20, 21]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("northpoint_1238_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("northpoint_1238_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("northpoint_1238_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("northpoint_1238_pair", "Exact supplied 1238 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("northpoint_1238_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("northpoint_1238_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("northpoint_1238_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("northpoint_1238_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("northpoint_1238_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("northpoint_1238_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("northpoint_1238_factory_shape", "Factory did not retain the exact roof result.", wall_record)
	var roof_root := Node3D.new()
	wall_root.remove_child(roof_mesh)
	roof_root.add_child(roof_mesh)
	wall_root.remove_child(roof_source)
	roof_root.add_child(roof_source)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Northpoint1238LiveWall" if wall else "Northpoint1238LiveRoof"
