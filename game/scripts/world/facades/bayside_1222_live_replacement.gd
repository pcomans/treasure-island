class_name Bayside1222LiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1222 Bayside source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/bayside_1222_live_factory.gd")
const ADAPTER_ID := "active-adapter:bayside-1222-live:building:w96215661:wall"
const SOURCE_KEY := "w96215661"
const WALL_KEY := "building:w96215661:wall"
const ROOF_KEY := "building:w96215661:roof"
const TARGET_CHUNK_ID := "x_-2__z_-3"
const EXPECTED_FACTORY_SHA256 := "639a5ce1cce22b5635f3bccfe6cb1c7764988662b826f0c25c4183fe2ec787f9"
const EXPECTED_CONFIG_SHA256 := "65a67decfd83b376209c8c8f80ab7f456be2006a8a907c2f36211c3e863e0ab2"
const MAPPED_RUNS := [10,11,12,13,14,15,16,19]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("bayside_1222_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("bayside_1222_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("bayside_1222_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("bayside_1222_pair", "Exact supplied 1222 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("bayside_1222_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("bayside_1222_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("bayside_1222_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("bayside_1222_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("bayside_1222_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("bayside_1222_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("bayside_1222_factory_shape", "Factory did not retain both original roof resources.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Bayside1222LiveWall" if wall else "Bayside1222LiveRoof"
