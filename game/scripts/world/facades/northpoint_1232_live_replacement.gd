...
## Proposed atomic live adapter for the reviewed 1232 Northpoint source pair.
...
const ADAPTER_ID := "active-adapter:northpoint-1232-live:building:w96215673:wall"
const SOURCE_KEY := "w96215673"
const WALL_KEY := "building:w96215673:wall"
const ROOF_KEY := "building:w96215673:roof"
const TARGET_CHUNK_ID := "x_-2__z_-3"
const EXPECTED_FACTORY_SHA256 := "05a93c31bbdfe0e9bf06530e02a33f49826f3af573e4128eaaa8d127cd686910"
const EXPECTED_CONFIG_SHA256 := "609436c6ee4f3f0170ed02b93ada28f927d67401aeb677c03a3dab9d76109f98"
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("northpoint_1232_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("northpoint_1232_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("northpoint_1232_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("northpoint_1232_pair", "Exact supplied 1232 wall and roof are required.", {})
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("northpoint_1232_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("northpoint_1232_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("northpoint_1232_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("northpoint_1232_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("northpoint_1232_factory_shape", "Factory did not retain both original roof resources.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Northpoint1232LiveWall" if wall else "Northpoint1232LiveRoof"
