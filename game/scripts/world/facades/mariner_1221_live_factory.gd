extends Node3D
## Preserved accepted 1221 revision002 construction behind live caller callbacks.
...
const GRASS := preload("res://game/resources/textures/world/polyhaven/sparse_grass/sparse_grass_diff_1k.jpg")
const GRASS_ROUGH := preload("res://game/resources/textures/world/polyhaven/sparse_grass/sparse_grass_rough_1k.jpg")
const SHINGLE_CODE := """shader_type spatial;
...
uniform vec4 warm_low : source_color;
uniform vec4 warm_high : source_color;
uniform float warm_station_max;
uniform vec2 warm_axis;
varying float source_x;
void vertex() { source_x = dot(VERTEX.xz, warm_axis); }
void fragment() {
    float tone = clamp(dot(texture(source_albedo, UV * 1.2).rgb, vec3(0.2126, 0.7152, 0.0722)) * 3.0, 0.0, 1.0);
    vec2 cell = UV / vec2(0.42, 0.19);
    cell.x += mod(floor(cell.y), 2.0) * 0.5;
    vec2 edge = min(fract(cell), 1.0 - fract(cell));
    vec2 aa = max(fwidth(cell), vec2(0.002));
    float joint = 1.0 - smoothstep(0.015, 0.015 + aa.y, edge.y);
    joint = max(joint, (1.0 - smoothstep(0.008, 0.008 + aa.x, edge.x)) * 0.55);
    float attenuation = 1.0 - smoothstep(0.25, 0.65, max(aa.x, aa.y));
    float warm = 1.0 - step(warm_station_max, source_x);
    vec3 low = mix(low_color.rgb, warm_low.rgb, warm);
    vec3 high = mix(high_color.rgb, warm_high.rgb, warm);
    ALBEDO = mix(low, high, tone) * (1.0 - joint * attenuation * 0.16);
    ROUGHNESS = 0.95;
    METALLIC = 0.0;
}
"""
const GROUND_BUCKETS := ["DrapedEntryPaths", "FootwaySurface", "ParkingSurface", "PlantedFrontage"]
const ROOF_BUCKET := "ObservedPublicRoof"
const SELF_PATH := "res://game/scripts/world/facades/mariner_1221_live_factory.gd"
const CONFIG_PATH := "res://game/resources/facades/mariner_1221_study_002.json"
...
func configure_records(wall: Dictionary, roof: Dictionary, source_builder: Callable, tangent_builder: Callable, baseline: bool = false) -> Dictionary:
	...
	for label: String in ["ObservedPublicRoof",  "CarportRoof", "CarportPosts", "CarportFascia", "NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures", "PaleCompleteTrim", "BlueOpaqueGlazing", "DarkOpaqueGlazing", "MaroonClosedDoors",   "LocalPrivacyScreens", "PaleDrainageAndFascia", "DoorHardware", "DrapedEntryPaths", "FootwaySurface", "ParkingSurface", "PlantedFrontage"]:
	...
	if not baseline:
		...
		for label: String in _buckets:
			if _buckets[label].indices.is_empty(): continue
		...
		body.set_meta("source_keys", ["w96215682"])
...
func _append_zone(frame: Dictionary, facade: Dictionary) -> void:
	...
	for entry: Dictionary in frame.entries:
		...
		var door: Dictionary = _buckets.MaroonClosedDoors
	...
	for pipe: Dictionary in frame.pipes:
		...
		for y in [bottom + .42, band + .11, top - .3]:
...
func _append_upper_volume(frame: Dictionary, facade: Dictionary) -> void:
	# The observed front source runs form three near-coplanar chains; plain closures stay within their span.
...
func _add_projected_receiver(wall: Dictionary) -> void:
	...
	body.set_meta("source_run_indices", [0,1,2,4,5,6,9,10,11,12])
...
func _append_path(path: Dictionary, label: String = "DrapedEntryPaths") -> void:
	...
	# The four paths are clipped to actual LAND triangles and the existing road
...
func _append_carport(carport: Dictionary) -> void:
	...
	KIT.append_box(_buckets.CarportFascia, _point(frame,(s0+s1)/2.0,float(carport.roof_y)+.025,d1-.035),tangent,normal,s1-s0,.23,.085)
	KIT.append_box(_buckets.CarportPosts, _point(frame,(s0+s1)/2.0,float(carport.roof_y)+.15,d1-.035),tangent,normal,s1-s0,.027,.09)
...
static func _surface_material(label: String, cfg: Dictionary) -> Material:
	if label == "CarportFascia":
		var fascia := StandardMaterial3D.new()
		fascia.resource_name = "1221_weathered_gray_canopy_fascia"
		fascia.albedo_texture = CONCRETE
		fascia.albedo_color = Color(.72,.72,.65,1.0)
		fascia.roughness = .96
		fascia.uv1_scale = Vector3(.7,.7,1.0)
		fascia.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC
		return fascia
	if label == ROOF_BUCKET:
		var roof := ShaderMaterial.new()
		var shader := Shader.new(); shader.code = SHINGLE_CODE
		roof.shader = shader
		roof.resource_name = "1221_observed_shingle_planes_gray_and_warm_left"
		...
		roof.set_shader_parameter("warm_low", _color(cfg.facade.roof_warm_low_rgb))
		roof.set_shader_parameter("warm_high", _color(cfg.facade.roof_warm_high_rgb))
		roof.set_shader_parameter("warm_station_max", float(cfg.facade.warm_roof_station_max))
		roof.set_shader_parameter("warm_axis", Vector2(float(cfg.facade.warm_roof_axis[0]), float(cfg.facade.warm_roof_axis[1])))
		return roof
	if label in GROUND_BUCKETS:
		# Luminance remapping exposes actual material colors without emission or baked illumination.
		var ground := ShaderMaterial.new()
		var shader := Shader.new()
		shader.code = LAWN_TONE.code.replace("ROUGHNESS = texture(lawn_roughness, sample_uv).r;", "ROUGHNESS = max(0.88, texture(lawn_roughness, sample_uv).r);")
		ground.shader = shader
		ground.resource_name = "1221_actual_grade_" + label
		var lawn := label == "PlantedFrontage"
		var parking := label == "ParkingSurface"
		ground.set_shader_parameter("lawn_albedo", GRASS if lawn else (ASPHALT if parking else CONCRETE))
		ground.set_shader_parameter("lawn_roughness", GRASS_ROUGH if lawn else (ASPHALT_ROUGH if parking else CONCRETE_ROUGH))
		var low := Color(.24,.34,.12,1.0) if lawn else (Color(.22,.23,.22,1.0) if parking else Color(.53,.54,.48,1.0))
		var high := Color(.48,.58,.25,1.0) if lawn else (Color(.39,.40,.38,1.0) if parking else Color(.78,.78,.70,1.0))
		ground.set_shader_parameter("dark_color", low)
		ground.set_shader_parameter("light_color", high)
		ground.set_shader_parameter("luminance_gain", 5.0)
		return ground
	var colors: Dictionary = {"CarportRoof": cfg.materials.carport_roof_rgb, "CarportPosts": cfg.materials.carport_post_rgb, "PaleCompleteTrim": cfg.materials.trim_rgb, "BlueOpaqueGlazing": cfg.materials.blue_glass_rgb, "DarkOpaqueGlazing": cfg.materials.dark_glass_rgb, "MaroonClosedDoors": cfg.materials.door_rgb, "PlumClosedDoors": cfg.materials.plum_door_rgb, "PaleClosedDoor": cfg.materials.pale_door_rgb, "LocalPrivacyScreens": cfg.materials.screen_rgb, "PaleDrainageAndFascia": cfg.materials.trim_rgb, "DoorHardware": cfg.materials.metal_rgb, "DrapedEntryPaths": cfg.materials.path_rgb}
	if label in ["NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures"]:
		...
		material.resource_name = "1221_pale_horizontal_siding"
	...
	material.resource_name = "1221_" + label
