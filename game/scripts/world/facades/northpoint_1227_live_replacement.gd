class_name Northpoint1227LiveReplacement
extends RefCounted

## Proposed atomic live adapter for the reviewed 1227 Northpoint source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/northpoint_1227_live_factory.gd")
const ADAPTER_ID := "active-adapter:northpoint-1227-live:building:w96215653:wall"
const SOURCE_KEY := "w96215653"
const WALL_KEY := "building:w96215653:wall"
const ROOF_KEY := "building:w96215653:roof"
const TARGET_CHUNK_ID := "x_-2__z_-4"
const EXPECTED_FACTORY_SHA256 := "6492b8e48226ed09c7e7171bd7fd51e25b8e347c38d87d2d0931b2b3337fb2f4"
const EXPECTED_CONFIG_SHA256 := "2e8573eb855c110c0d13545d63f8fd467a25a68613d16b91cb1336462baadde7"
const MAPPED_RUNS := [4,5,6,7,8,9,10,11,13,14,15,16,17]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("northpoint_1227_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("northpoint_1227_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("northpoint_1227_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("northpoint_1227_pair", "Exact supplied 1227 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("northpoint_1227_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("northpoint_1227_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("northpoint_1227_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("northpoint_1227_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("northpoint_1227_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("northpoint_1227_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("northpoint_1227_factory_shape", "Factory did not retain both original roof resources.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Northpoint1227LiveWall" if wall else "Northpoint1227LiveRoof"
