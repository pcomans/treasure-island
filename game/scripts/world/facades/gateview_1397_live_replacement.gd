...
## Proposed atomic live adapter for the reviewed 1397 Gateview source pair.
...
const FACTORY := preload("res://game/scripts/world/facades/gateview_1397_live_factory.gd")
const ADAPTER_ID := "active-adapter:gateview-1397-live:building:w96215670:wall"
const SOURCE_KEY := "w96215670"
const WALL_KEY := "building:w96215670:wall"
const ROOF_KEY := "building:w96215670:roof"
const TARGET_CHUNK_ID := "x_-1__z_-3"
const EXPECTED_FACTORY_SHA256 := "ce6223bf527c74431dc794bfc9ab7211feabd881e9d080e318a2f712547e4a91"
const EXPECTED_CONFIG_SHA256 := "97919d0168bab83ff0f7118cf00fb0bbea1eb062c731d2b956de896a1330616a"
const MAPPED_RUNS := [17, 18, 19, 20, 23, 24, 25, 26, 27, 28, 30, 31, 32, 33, 34, 35, 36, 37]
...
static func prepare_chunk_records(chunk: Dictionary) -> Dictionary:
	if not (chunk.get("records", null) is Array):
		return _failure("gateview_1397_chunk_records", "Missing supplied records.", {})
	...
	for value: Variant in chunk.records:
		...
		if claims_record(record) or SOURCE_KEY in source_keys:
			if not claims_record(record):
				return _failure("gateview_1397_source_alias", "Unexpected target source alias.", record)
			var key := str(record.get("object_key", ""))
			if records.has(key):
				return _failure("gateview_1397_duplicate_record", "Duplicate target pair member.", record)
			records[key] = record
	...
	if str(chunk.get("chunk_id", "")) != TARGET_CHUNK_ID or not _pair_matches(records):
		return _failure("gateview_1397_pair", "Exact supplied 1397 wall and roof are required.", {})
...
static func build_chunk_plan(prepared: Dictionary, source_builder: Callable, tangent_builder: Callable) -> Dictionary:
	...
	if not _pair_matches(records) or not _dependencies_match():
		return _failure("gateview_1397_preflight", "Pinned factory, config, or source pair drifted.", {})
	var built := FACTORY.build_for_records(records[WALL_KEY], records[ROOF_KEY], source_builder, tangent_builder)
	if not bool(built.get("ok", false)):
		return _failure("gateview_1397_factory", str(built.get("message", "Factory failed.")), records[WALL_KEY])
...
static func consume_record(record: Dictionary, plan: Dictionary) -> Dictionary:
	...
	if not claims_record(record) or not bool(plan.get("ok", false)) or not bool(plan.get("contains_target", false)):
		return _failure("gateview_1397_unprepared", "Validated supplied plan required.", record)
	if not FACTORY.matches_record_pair(
		...
		return _failure("gateview_1397_consumed_drift", "Consumed row differs from the frozen source pair.", record)
	...
	if not pending.has(key) or not results.has(key):
		return _failure("gateview_1397_duplicate_consume", "Pair member missing or consumed twice.", record)
...
static func _split_factory_result(built: Dictionary, wall_record: Dictionary) -> Dictionary:
	...
	if wall_root == null:
		return _failure("gateview_1397_factory_node", "Factory returned no root.", wall_record)
	...
	if roof_mesh == null or roof_source == null:
		wall_root.free()
		return _failure("gateview_1397_factory_shape", "Factory did not retain both original roof resources.", wall_record)
...
static func _apply_live_metadata(root: Node3D, key: String, wall: bool) -> void:
	root.name = "Gateview1397LiveWall" if wall else "Gateview1397LiveRoof"
