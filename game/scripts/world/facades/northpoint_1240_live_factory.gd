extends Node3D
## Proposed exact accepted002 construction behind live caller callbacks; not attached.
...
const SELF_PATH := "res://game/scripts/world/facades/northpoint_1240_live_factory.gd"
const CONFIG_PATH := "res://game/resources/facades/northpoint_1240_study.json"
...
const TARGET_RUNS := [15, 16, 18, 19, 20, 21, 23, 24, 25]
...
func configure_records(wall: Dictionary, roof: Dictionary, source_builder: Callable, tangent_builder: Callable, baseline: bool = false) -> Dictionary:
	...
	if str(cfg.get("schema_version", "")) != "ti.northpoint-1240-study/1" or str(cfg.target.source_key) != "w96215688":
		return {"ok": false, "message": "This coherent study is 1240 only."}
	...
	if not baseline:
		...
		body.set_meta("source_keys", ["w96215688"])
...
static func _surface_material(label: String, cfg: Dictionary) -> Material:
	if label == "CarportFascia":
		...
		fascia.resource_name = "1240_weathered_gray_canopy_fascia"
	...
	if label == ROOF_BUCKET:
		...
		roof.resource_name = "1240_shallow_gray_observed_roof"
	...
	if label in GROUND_BUCKETS:
		...
		ground.resource_name = "1240_actual_grade_" + label
	...
	if label in ["NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures"]:
		...
		material.resource_name = "1240_pale_horizontal_siding"
	...
	material.resource_name = "1240_" + label
