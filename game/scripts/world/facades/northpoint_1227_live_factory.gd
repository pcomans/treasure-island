extends Node3D
## Byte-preserved 1227 study construction behind the live caller callbacks.
...
const SELF_PATH := "res://game/scripts/world/facades/northpoint_1227_live_factory.gd"
const CONFIG_PATH := "res://game/resources/facades/northpoint_1227_study.json"
const CHUNK_PATH := "res://generated/world/chunks/x_-2__z_-4.json"
const WALL_KEY := "building:w96215653:wall"
const ROOF_KEY := "building:w96215653:roof"
const EXPECTED_CHUNK_SHA256 := "932c924b845a382d746e7900710c0c4e8b7603ab205cb07173ad51933c1341b4"
const TARGET_RUNS := [4,5,6,7,8,9,10,11,13,14,15,16,17]
const PROTECTED_RUNS := [0,1,2,3,12,18,19,20,21]
const PHYSICAL_BUCKETS := ["ObservedPublicRoof","NorthGableSiding","CarportRoof","CarportPosts","ProjectedUpperClosures","PaleCompleteTrim","BlueOpaqueGlazing","DarkOpaqueGlazing","BlueGrayClosedDoors","PaleClosedDoor","LocalPrivacyScreens","PaleDrainageAndFascia","DoorHardware"]
...
func configure_records(wall: Dictionary, roof: Dictionary, source_builder: Callable, tangent_builder: Callable, baseline: bool = false) -> Dictionary:
	...
	if str(cfg.get("schema_version", "")) != "ti.northpoint-1227-study/1" or str(cfg.target.source_key) != "w96215653":
		return {"ok": false, "message": "This coherent study is 1227 only."}
	...
	for label: String in ["ObservedPublicRoof", "NorthGableSiding", "CarportRoof", "CarportPosts", "NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures", "PaleCompleteTrim", "BlueOpaqueGlazing", "DarkOpaqueGlazing", "BlueGrayClosedDoors", "PaleClosedDoor", "LocalPrivacyScreens", "PaleDrainageAndFascia", "DoorHardware", "DrapedEntryPaths", "FootwaySurface", "ParkingSurface"]:
	...
	if not baseline:
		...
		_append_gable(cfg.public_roof.north_gable_triangles)
		...
		body.set_meta("source_keys", ["w96215653"])
...
func _append_zone(frame: Dictionary, facade: Dictionary) -> void:
	...
	for entry: Dictionary in frame.entries:
		...
		var door: Dictionary = _buckets.PaleClosedDoor if str(entry.door_tone) == "pale" else _buckets.BlueGrayClosedDoors
...
func _add_projected_receiver(wall: Dictionary) -> void:
	...
	body.set_meta("source_run_indices", [4,5,6,7,9,10,11,13,14,15,16])
...
func _append_skin(wall: Dictionary, run: int) -> void:
	...
	if run == 11:
		# Retained source-ray bound: hidden northern tail gets no added skin.
		points[1] = points[0].lerp(points[1], 0.6337911232803874)
		points[2] = points[3].lerp(points[2], 0.6337911232803874)
...
func _append_gable(triangles: Array) -> void:
	var bucket: Dictionary = _buckets.NorthGableSiding
	for triangle: Array in triangles:
		var a := _v(triangle[0]); var b := _v(triangle[1]); var c := _v(triangle[2])
		var normal := (b-a).cross(c-a).normalized()
		var base: int = bucket.vertices.size()
		for point: Vector3 in [a,b,c]:
			bucket.vertices.append(point); bucket.normals.append(normal)
			bucket.uvs.append(Vector2(point.x,point.y))
		bucket.indices.append_array([base,base+2,base+1])
...
static func _surface_material(label: String, cfg: Dictionary) -> Material:
	if label == ROOF_BUCKET:
		...
		roof.resource_name = "1227_shallow_gray_observed_roof"
	...
	if label in GROUND_BUCKETS:
		...
		ground.resource_name = "1227_actual_grade_" + label
		...
		return ground
	var colors: Dictionary = {"CarportRoof": cfg.materials.carport_roof_rgb, "CarportPosts": cfg.materials.carport_post_rgb, "PaleCompleteTrim": cfg.materials.trim_rgb, "BlueOpaqueGlazing": cfg.materials.blue_glass_rgb, "DarkOpaqueGlazing": cfg.materials.dark_glass_rgb, "BlueGrayClosedDoors": cfg.materials.door_rgb, "PaleClosedDoor": cfg.materials.pale_door_rgb, "LocalPrivacyScreens": cfg.materials.screen_rgb, "PaleDrainageAndFascia": cfg.materials.trim_rgb, "DoorHardware": cfg.materials.metal_rgb, "DrapedEntryPaths": cfg.materials.path_rgb}
	if label in ["NearHorizontalSiding", "ProjectedUpperSiding", "ProjectedUpperClosures", "NorthGableSiding"]:
		...
		material.resource_name = "1227_pale_horizontal_siding"
	...
	material.resource_name = "1227_" + label
