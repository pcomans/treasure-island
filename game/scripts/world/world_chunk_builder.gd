...
func _build_record(record: Dictionary, is_context: bool, chapel_plan: Dictionary = {}) -> Dictionary:
	...
	var surface_plan := ACCEPTED_MATERIAL_RUN_TRIALS.partition(record, indices, placeholder_material) \
		if not is_context else {"ok": true, "surfaces": [{"name": "generated_record", "indices": indices, "material": placeholder_material}]}
